# A Mid-to-Late Token-Identity/Readout Split in Instruction-Tuned Language Models
...
Instruction tuning changes what language models say, but less is known about how it changes the layerwise route from candidate tokens to final next-token probabilities. We study five dense model families with paired pretrained and instruction-tuned checkpoints using matched-prefix histories, symmetric early/middle/late MLP grafts and swaps, and first-divergence-token counterfactuals. The headline result is a **mid-to-late token-identity/readout split**. At prefixes where pretrained and instruction-tuned models first choose different tokens, middle-layer substitutions transfer opposite-model token identity more than late substitutions (`26.0%` versus `17.6%` IT-token transfer in a pretrained host), while late instruction-tuned MLPs give the strongest direct native support to the instruction-tuned token but are weak when inserted into a pretrained upstream state (`+0.789` versus `+0.0035` logits). A convergence-gap signature motivates this split: instruction-tuned models stabilize later to their own final next-token distribution, and matched-prefix graft/swap interventions localize the delayed-stabilization component most strongly to late MLPs, with matched random controls near zero. Thus post-training does not simply install a late assistant module. It changes a route: middle layers more strongly affect which PT- or IT-like token is exposed, and late MLPs more strongly affect whether that token wins once the upstream state is already IT-shaped. This gives mechanistic interpretability a cross-family protocol for studying how post-training effects become next-token probabilities, beyond final behavioral scores or isolated steering directions.
...
This paper argues for a single answer: **instruction tuning induces a mid-to-late identity/readout split in next-token formation**. Middle layers are more tied to which pretrained- or instruction-tuned-like token candidate is exposed. Late MLPs are more tied to making the selected candidate win the final next-token competition, but only when the upstream state is already instruction-tuned-like.
...
1. **Instruction tuning creates delayed stabilization.** Across paired dense PT/IT families, IT models stabilize later to their own final next-token distributions, and same-history JS replay shows PT/IT output distributions still differ when token histories are held fixed.
...
3. **Token identity and readout separate across depth.** First-divergence counterfactuals show that middle layers transfer PT-vs-IT token identity more than late layers, while late IT MLPs provide the largest native IT-token write-out and margin contribution once the upstream state is IT-shaped.

Prior work has studied late prediction sharpening, compared base and post-trained models on behavioral or representation directions, and localized safety or preference behavior to mid-layer bands. We combine matched-prefix histories, symmetric depth-localized MLP interventions, matched random controls, and first-divergence counterfactuals to ask a different question: how does post-training change the route from candidate tokens to final next-token predictions?

The main paper reports pooled results for five dense PT/IT families: Gemma 3 4B, Llama 3.1 8B, Qwen 3 4B, Mistral 7B, and OLMo 2 7B. A separate MoE/SFT-only checkpoint is retained as an appendix robustness side case, not as part of the main dense-family claims. Appendix F collects scope boundaries, including endpoint and probe dependence, prompt formatting, weight-change magnitude, MoE routing, and the fact that window-level grafts localize causal leverage on measured readouts rather than recovering a complete named circuit.
...
All main-text claims below therefore refer to the five dense families; the separate MoE/SFT-only side case is reported only in the appendices.
...
We use three data regimes, each tied to one role in the argument.
...
The results build the identity/readout split in four steps: instruction-tuned models stabilize later; late MLP substitutions most strongly control that delayed-stabilization signature; first-divergence tests show that middle layers affect token identity while late layers affect readout; and free-running behavior changes in the predicted direction.
...
Appendix A gives probe, commitment, per-family, and reverse-teacher robustness views.
...
### 3.3 Middle layers affect token identity; late layers affect readout
...
![Figure 4: The identity/readout split. First-divergence tests show that middle substitutions transfer PT-vs-IT token identity more, while late IT MLPs provide the strongest native IT-token readout and margin contribution once the upstream state is IT-shaped. The full MLP update carries the IT-vs-PT margin write-in; the negative-parallel residual component is only a geometric marker.](../results/paper_synthesis/exp20_exp21_handoff_synthesis.png)

The synthesis is the paper's headline. Middle layers are more diagnostic of which PT- or IT-like token candidate is exposed. Late MLPs are more diagnostic of final readout: they provide the largest native IT-token write-out and margin contribution, and the same late window controls the delayed-stabilization signature from Section 3.2. The convergence gap is therefore not the birthplace of instruction-tuned behavior; it is the readout cost of making selected instruction-tuned candidates dominate the final next-token distribution. Appendix A gives the token-flow chronology and residual-opposition decomposition.
...
Late sharpening is not new by itself. The logit-lens and tuned-lens line of work makes late-layer prediction dynamics visible (nostalgebraist, 2020; Belrose et al., 2023), Lad et al. (2024) frames late computation as residual sharpening after earlier candidate construction, and Joshi et al. (2025) studies late confidence calibration after decision certainty has emerged. Our contribution is not another claim that late layers sharpen. It is a PT/IT model-diffing result: post-training changes the route from token candidates to final next-token probabilities, and first-divergence counterfactuals separate middle-layer token identity from late-layer margin/readout.

Post-training model-diffing and activation-patching papers ask nearby questions. Wu et al. (2024) studies instruction-conditioned behavioral shift, Du et al. (2025) compares base and post-trained models across knowledge, truthfulness, refusal, and confidence, and Prakash et al. (2024) uses cross-model activation patching to study entity tracking. We use related intervention logic, but the target is different: matched-prefix PT/IT prediction dynamics under symmetric depth-localized MLP grafts and swaps, plus first-divergence tests that distinguish identity from readout.

Behavioral-direction papers explain why the late readout effect should not be confused with a complete assistant module. Arditi et al. (2024), Lu et al. (2026), Stolfo et al. (2024), Turner et al. (2023), Zou et al. (2023), and Panickssery et al. (2024) show that activation-space directions can control refusal, assistant persona, instruction following, or high-level behavior. Li et al. (2025) and Chaudhury (2025) also find safety or preference effects in middle bands. Our first-divergence result is compatible with that picture: middle layers are more tied to which behaviorally relevant token candidate is selected, while late MLPs are where our readouts find the clearest final-token write-in.

Taken together, the evidence supports a window-level account: IT models stabilize later to their own final predictions, late MLP substitutions most strongly control that delayed-stabilization signature, and first-divergence tests reveal an identity/readout split. The paper does not claim to recover a named middle-layer feature feeding a named late write-in circuit. It identifies a cross-family route that future circuit work can make more surgical.

## 5. Limitations and Next Tests

The convergence gap is endpoint-relative by design: it asks how far each layer is from the same model's eventual next-token prediction. Same-history JS replay and raw-lens variants reduce but do not eliminate all probe and endpoint concerns. The main causal claims are therefore pinned to the conjunction of native discovery, same-history replay, graft/swap localization, matched random controls, and first-divergence token tests, not to any one metric alone.
...
In short, post-training changes the layerwise process by which models settle on next tokens: middle layers more strongly affect PT-vs-IT token identity, and late MLPs help selected IT-like candidates win once the upstream state is already IT-shaped.
...
DeepSeek-V2-Lite is retained as a side case wherever the experiments were run, but it is not pooled into the main dense-family intervention or behavioral claims. The figures and tables above include DeepSeek panels or rows when available; Appendix F states the interpretation boundary.
...
|---|---|---|---|
| Six-family final-half convergence gap | `bash scripts/reproduce/reproduce_claims_from_summaries.sh` | `results/exp09_cross_model_observational_replication/data/convergence_gap_values.json` | tuned `0.398`; raw `0.729`; raw excluding Gemma and DeepSeek `0.712` |
