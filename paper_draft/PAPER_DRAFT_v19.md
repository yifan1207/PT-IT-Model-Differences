# Coarse-Window Causal Evidence for a Mid-to-Late Token-Identity/Readout Split in Instruction-Tuned Language Models
...
Instruction tuning changes what language models say, but less is known about how it changes the layerwise route by which a pretrained model and its instruction-tuned descendant first diverge on a next token. We study five dense model families with paired pretrained and instruction-tuned checkpoints using matched-prefix histories, endpoint-matched trajectory controls, symmetric early/middle/late MLP grafts and swaps, and first-divergence-token counterfactuals. The headline result is **coarse-window causal evidence for a mid-to-late division of causal leverage over token identity and final readout**. At prefixes where pretrained and instruction-tuned models first choose different tokens, middle-window substitutions transfer opposite-model token identity more than late substitutions (`26.0%` versus `17.6%` IT-token transfer in a pretrained host), while late instruction-tuned MLPs give the strongest direct native support to the instruction-tuned token but are weak when inserted into a pretrained upstream state (`+0.789` versus `+0.0035` logits). A convergence-gap signature motivates this split: instruction-tuned models stabilize later to their own final next-token distribution, and the measured late gap remains after matching token steps on final entropy, final top-1 confidence, and final top-1/top-2 margin (`+0.425` raw, `+0.762` tuned nats). Matched-prefix graft/swap interventions then localize the delayed-stabilization component most strongly to late MLP windows, with matched random controls near zero. Thus the contribution is not another claim that predictions sharpen late, or that instruction tuning creates an isolated behavioral direction. It is a paired-checkpoint route decomposition at the granularity of depth windows: middle windows more strongly affect which PT- or IT-like token is exposed, and late MLP windows more strongly affect whether that token wins once the upstream state is already IT-shaped. This gives mechanistic interpretability a cross-family protocol for studying how post-training effects become next-token probabilities, beyond final behavioral scores or isolated steering directions.
...
This paper argues for a single window-level answer: **instruction tuning induces a mid-to-late division of causal leverage in next-token formation**. The claim is not that late sharpening or mid-layer behavioral localization is new in isolation, and it is not that we recover a complete named circuit for instruction tuning. The new object is the paired PT/IT disagreement point: at the first prefix where a pretrained model and its instruction-tuned descendant choose different next tokens, middle windows are more tied to which PT- or IT-like candidate is exposed, while late MLP windows are more tied to making the selected candidate win the final next-token competition once the upstream state is already instruction-tuned-like.

The motivating signature is delayed stabilization. For every layer, we decode a next-token distribution and compare it to the model's own final distribution. This **convergence gap** asks how far a layer is from the model's eventual next-token prediction. Instruction-tuned models stay farther from their own final predictions for longer than pretrained models do, and matched-prefix MLP grafts and swaps localize the delayed-stabilization component most strongly to late MLPs. The key additional step is first-divergence analysis: at the first token where PT and IT would choose differently, we can ask whether a depth intervention changes token identity or only final margin/readout.

This paper makes three claims:

1. **Instruction tuning creates delayed stabilization.** Across paired dense PT/IT families, IT models stabilize later to their own final next-token distributions. The late KL gap remains after matching on measured final endpoint entropy, confidence, and top-1/top-2 margin, and same-history JS replay shows PT/IT output distributions still differ when token histories are held fixed.
2. **Late MLP substitutions most strongly control the delayed-stabilization signature.** Under identical histories, grafting late IT MLPs into a PT host recreates the delay, swapping late PT MLPs into an IT host collapses it, and matched random late perturbations are near zero.
3. **Token identity and readout show a coarse depth division.** First-divergence counterfactuals show that middle windows transfer PT-vs-IT token identity more than late windows, while late IT MLP windows provide the largest native IT-token write-out and margin contribution once the upstream state is IT-shaped.

Prior work gives nearby pieces: late prediction sharpening and confidence calibration, base-versus-post-training comparisons, behavioral steering directions, and mid-layer safety or preference localization. We do not treat any one of those ingredients as new. We combine matched-prefix histories, symmetric depth-localized MLP interventions, matched random controls, and first-divergence counterfactuals to ask a narrower question those papers do not directly answer: when a PT model and its IT descendant first disagree, which depth windows change token identity, and which windows change final margin/readout?

The main paper reports pooled results for five dense PT/IT families: Gemma 3 4B, Llama 3.1 8B, Qwen 3 4B, Mistral 7B, and OLMo 2 7B. We also ran DeepSeek-V2-Lite as a MoE/SFT-only side case, but do not pool it with dense models because MLP grafts in MoEs also perturb routing and expert selection. Appendix F collects scope boundaries, including endpoint and probe dependence, prompt formatting, weight-change magnitude, MoE routing, and the fact that window-level grafts localize causal leverage on measured readouts rather than recovering a complete named circuit.
...
The main experiments use five dense PT/IT model families. We use `instruction tuning` and `IT` as readable shorthand for instruction-following post-trained descendants of PT checkpoints; the recipes are heterogeneous, so the comparison is PT-versus-post-training model diffing rather than isolation of one training algorithm.
...
Four companion readouts prevent that metric from carrying too much alone. Endpoint-matched token-step controls test whether the late KL gap remains after matching on final entropy, top-1 confidence, and top-1/top-2 margin. Matched-prefix same-layer JS compares PT and IT output distributions under identical teacher tokens without using either model's final endpoint. Equal-width early/middle/late MLP grafts and swaps test which depth window has the largest causal leverage on delayed stabilization under identical histories, with matched random residual-projection controls for generic perturbation. First-divergence token tests separate token identity from final margin/readout at prefixes where PT and IT would choose different next tokens.
...
### 3.3 Middle windows affect token identity; late MLP windows affect readout
...
The margin tests give the readout side. In native IT-host swaps, replacing late IT MLPs with PT MLPs produces the largest single-window IT-vs-PT margin drop (`13.25` logits, 95% CI `[12.91, 13.61]`) relative to early (`11.53`, 95% CI `[11.20, 11.88]`) and middle (`12.01`, 95% CI `[11.66, 12.35]`) swaps. In pure IT runs, MLP support for the IT divergent token is overwhelmingly late: early `-0.041`, middle `+0.021`, late `+0.789` logits. The PT-to-IT change in that support is also concentrated late: early `+0.034`, middle `+0.070`, late `+0.715` logits.

The late effect is context-gated. Adding late IT MLPs to a PT host changes the same fixed-prefix IT-vs-PT margin by only `+0.0035` logits (95% CI `[-0.001, +0.009]`), while removing late IT MLPs from an IT host produces a much larger margin loss. A 2x2 source decomposition makes this explicit: upstream IT context (`+0.403`, 95% CI `[+0.360, +0.444]`) is larger than the late-weight main effect (`+0.148`, 95% CI `[+0.139, +0.157]`), with positive interaction (`+0.288`, 95% CI `[+0.277, +0.301]`). Late readout is real, but it requires an IT-shaped upstream state.

![Figure 4: Coarse-window identity/readout leverage. First-divergence tests show that middle substitutions transfer PT-vs-IT token identity more, while late IT MLP windows provide the strongest native IT-token readout and margin contribution once the upstream state is IT-shaped. The full MLP update carries the IT-vs-PT margin write-in; the negative-parallel residual component is only a geometric marker.](../results/paper_synthesis/exp20_exp21_handoff_synthesis.png)

The synthesis is the paper's headline. At the granularity tested here, middle windows are more diagnostic of which PT- or IT-like token candidate is exposed. Late MLP windows are more diagnostic of final readout: they provide the largest native IT-token write-out and margin contribution, and the same late window controls the delayed-stabilization signature from Section 3.2. The convergence gap is therefore not the birthplace of instruction-tuned behavior; it is a signature of the late-window cost of making selected instruction-tuned candidates dominate the final next-token distribution. Appendix A gives the token-flow chronology and residual-opposition decomposition.

### 3.4 Behavioral consequence check

The same intervention family changes natural-decoding outputs in the predicted direction, but this is consequence evidence rather than the core mechanistic proof. Under LLM judging on the dense-family 600-prompt behavioral subset, replacing late IT MLPs with PT MLPs produces the strongest IT-side degradation on assistant register (`G2`, `+0.42` worsening, 95% CI `[+0.27, +0.62]`) and benign false-refusal control (`S2`, `+0.05`, 95% CI `[+0.02, +0.10]`). Grafting late IT MLPs into PT produces smaller but measurable movement: blind pairwise judging prefers PT with late IT MLPs grafted in over the PT baseline on `56.3%` of resolved assistant-register items and `63.8%` of resolved benign-safety items. A completed two-rater blind human audit agrees directionally on the four primary pairwise contrasts, while high tie/both-bad rates and heterogeneous agreement keep the claim bounded. Appendix B gives the judge design, Figure B1, human-audit tables, and κ caveats.

## 4. Related Work and Discussion

Late sharpening is not new by itself. The logit-lens and tuned-lens line of work makes late-layer prediction dynamics visible (nostalgebraist, 2020; Belrose et al., 2023), Lad et al. (2024) frames late computation as residual sharpening after earlier candidate construction, and Joshi et al. (2025) studies late confidence calibration after decision certainty has emerged. We use the convergence gap as an aggregate signature of delayed stabilization, not as the novelty claim in isolation. The contribution is the PT/IT first-divergence decomposition: at the exact prefix where paired checkpoints choose different next tokens, depth interventions separate candidate identity from final margin/readout.

Post-training model-diffing and activation-patching papers ask nearby questions. Wu et al. (2024) studies instruction-conditioned behavioral shift, Du et al. (2025) compares base and post-trained models across knowledge, truthfulness, refusal, and confidence, and Prakash et al. (2024) uses cross-model activation patching to study entity tracking. We use related intervention logic, but the target is different: natural PT/IT next-token disagreement under matched-prefix control. This lets us ask whether post-training changes which token candidate is exposed, whether it changes the final readout margin, or both.

Behavioral-direction papers explain why the late readout effect should not be confused with a complete assistant module. Arditi et al. (2024), Lu et al. (2026), Stolfo et al. (2024), Turner et al. (2023), Zou et al. (2023), and Panickssery et al. (2024) show that activation-space directions can control refusal, assistant persona, instruction following, or high-level behavior. Li et al. (2025) and Chaudhury (2025) also find safety or preference effects in middle bands. Our first-divergence result is compatible with that picture rather than a foil to it: middle windows are more tied to which behaviorally relevant token candidate is selected, while late MLP windows are where our readouts find the clearest final-token write-in.

Taken together, the evidence supports a coarse-window account: IT models stabilize later to their own final predictions, late MLP substitutions most strongly control that delayed-stabilization signature, and first-divergence tests reveal a mid-to-late division of causal leverage over token identity and final readout. The paper does not claim to recover a named middle-layer feature feeding a named late write-in circuit. It identifies a cross-family route decomposition that future circuit work can make more surgical.

## 5. Limitations and Next Tests

The convergence gap is endpoint-relative by design: it asks how far each layer is from the same model's eventual next-token prediction. Endpoint-matched controls show that the measured late KL gap remains after balancing final entropy, confidence, and top-1/top-2 margin, while same-history JS replay removes the own-final endpoint from a companion readout. The main causal claims are therefore pinned to the conjunction of native discovery, endpoint-matched controls, same-history replay, graft/swap localization, matched random controls, and first-divergence token tests, not to any one metric alone.

The intervention evidence is window-level, not circuit-level. MLP grafts and swaps localize causal leverage on specified readouts, but they do not identify a complete feature circuit. The residual-opposition signature is also only a marker: the negative-parallel component does not directly carry the IT-vs-PT margin write-in, while the full late MLP update does. Exploratory dimensionality, attention-entropy, and Gemma transcoder analyses are appendix diagnostics rather than pillars of the claim.

The behavioral evidence is a consequence check. LLM judging and the completed human audit agree directionally on the primary pairwise contrasts, but high tie/both-bad rates and heterogeneous κ keep it from being a standalone behavioral benchmark. The internal localization result does not depend on those behavioral labels.

The next decisive test is context gating. Start from an IT host and selectively replace or interpolate the upstream residual state entering the late window while keeping late IT weights fixed; conversely, start from a PT host and supply IT-like upstream states while keeping late weights fixed. A higher-resolution matched-prefix rerun should store per-layer logits rather than only ranks/top-k sets, and a finer late-update decomposition should isolate which token-supporting or alternative-suppressing components recover delayed stabilization and IT-token margin.

In short, post-training changes the layerwise process by which models settle on next tokens: at the tested window level, middle layers more strongly affect PT-vs-IT token identity, and late MLPs help selected IT-like candidates win once the upstream state is already IT-shaped.
...
### A5: Same-history JS and identity/readout details
...
![Figure B1: Free-running behavioral consequence check. The same late intervention family produces measurable LLM-judge behavioral movement under natural decoding, but the figure is appendix material because behavior is a consequence check rather than the core identity/readout localization evidence.](../results/exp15_symmetric_behavioral_causality/plots/exp15_eval_core_600_t512_dense5/exp15_paper_behavior_main.png)
...
| Paper | PT↔post-trained descendants? | Cross-family paired checkpoints? | Identical-history internal comparison? | Symmetric depth-localized intervention? | First-divergence identity/readout? | Natural-decoding consequence test? |
|---|---|---|---|---|---|---|
...
| MLP write-out and context gating | same | `results/exp21_productive_opposition/exp21_full_productive_opposition_clean_20260426_053736/analysis/summary.json`; `analysis/effects.csv` | late IT-token support `+0.789`; PT-host late graft `+0.0035`; upstream `+0.403`, late-weight `+0.148`, interaction `+0.288` |
...
**Committed versus regenerated artifacts.** The repository commits paper-facing summaries and plots used for manuscript numbers: JSON/CSV/MD summaries, bootstrap confidence intervals, human-evaluation summaries, and final PNG figures. It does not commit large regenerated intermediates such as raw activation arrays (`*.npy`, `*.npz`), model/probe tensors (`*.pt`, `*.safetensors`), tuned-lens checkpoints, or multi-gigabyte raw per-token trace JSONL/GZ files. These are size exclusions, not confidentiality exclusions. The corresponding collection scripts, analysis scripts, prompt manifests, and GCS pointers are public.

**Large-artifact locations.** The raw-data mirror uses the `gs://pt-vs-it-results/` bucket. Tuned-lens probes are mirrored at `gs://pt-vs-it-results/tuned_lens_probes_v3/`. The Exp22 endpoint-control raw records are mirrored at `gs://pt-vs-it-results/exp22_endpoint_deconfounded_gap/exp22_full_dense5_raw_tuned_20260426_150029/`. The late random-control raw trace archive is mirrored at `gs://pt-vs-it-results/results/exp19_late_mlp_specificity_controls/exp19B_core120_h100x8_20260424_050421/`. The Exp20 dense-factorial raw archives are listed in `results/exp20_divergence_token_counterfactual/exp20_midlate_factorial_compact_20260424/GCS_URIS.txt`, including `gs://pt-vs-it-results/results/exp20_divergence_token_counterfactual/midlate_factorial_20260424_1116/` and `gs://pt-vs-it-results/results/exp20_divergence_token_counterfactual/full_runpod_20260423_2148_dense5_midlate_factorial_20260424/`. The supplementary minimal audit shard is mirrored, or bundled directly for review, under `gs://pt-vs-it-results/reproducibility/minimal_audit_shard/`.

**Hardware and runtime.** All full reruns use bf16 inference and deterministic greedy decoding unless a script states otherwise. The 4B-8B dense-family experiments fit on one 80GB A100/H100 per model job; DeepSeek-V2-Lite is reported separately because it is MoE/SFT-only. The full dense-5 causal and endpoint-control suite (Exp11/14/16/20/21/22, including trace collection and analysis) should be budgeted as hundreds of H100 GPU-hours and roughly 0.5-1.5 TB of transient disk if raw traces and per-token records are retained. The minimal 20-prompt shard needs one 80GB A100/H100, about 1-3 GPU-hours if regenerated from checkpoints, and about 20-80 GB local disk depending on whether all per-layer logits are kept. The summary audit is CPU-only and reads only committed JSON/CSV files.

**Tuned-lens reproducibility.** The main causal claims do not require tuned-lens checkpoints. The matched-prefix JS replay, graft/swap KL summaries, first-divergence identity/margin results, and finite-difference MLP write-out analyses are auditable from committed summaries and do not depend on trained probes. The tuned lens is used for the discovery visualization and commitment diagnostics, and the paper reports raw-lens sensitivity alongside it. Probe checkpoints are mirrored at `gs://pt-vs-it-results/tuned_lens_probes_v3/`; retraining uses `uv run python -m src.poc.cross_model.tuned_lens --model MODEL --variant {pt,it} --device cuda:N`. A complete dense-5 retrain is 10 PT/IT runs; including the DeepSeek side case is 12 runs. The full set takes about 4-6 wall-clock hours on 8x80GB A100/H100 GPUs with joint all-layer training.
