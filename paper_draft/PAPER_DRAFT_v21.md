# Context-Gated Late Readout in Dense Instruction-Tuned Language Models
...
We show, using depth-localized hybrid forward-pass interventions, that late computation in dense instruction-tuned language models depends strongly on upstream state; it is not well described as an autonomous late instruction module. We measure this at first-divergence prefixes: positions where a pretrained checkpoint (PT) and its post-trained descendant (IT), given identical history, first prefer different next tokens. In a residual-state x late-stack factorial across five dense PT/IT model families, the same IT late stack shifts the IT-vs-PT margin by `+3.21` logits from an IT-shaped upstream state but only `+0.57` logits from a PT upstream state, a `5.6x` asymmetry with positive interaction term `+2.64` logits (95% CI `[+2.54, +2.74]`); excluding Gemma leaves a four-family mean interaction of `+1.77` logits (95% CI `[+1.69, +1.86]`). A separate factual/reasoning extension preserves the central interaction (`+1.68` logits, 95% CI `[+1.59, +1.78]`) but flips the late-only term negative from PT upstream (`-1.41` logits, 95% CI `[-1.51, -1.31]`), showing that the sign of late-only readout is domain-dependent. To establish this, we combine endpoint-matched convergence trajectories, endpoint-free same-history replay, symmetric MLP grafts and swaps, matched random controls, and first-divergence token counterfactuals. The factorial result sits on a broader decomposition: middle-window substitutions transfer PT/IT token identity more than late substitutions (`26.0%` versus `17.6%` IT-token transfer in a PT host), while late MLP grafts and swaps have the largest tested leverage on an observational delayed-stabilization signature and matched random late perturbations are near zero (`+0.003` versus `+0.327` nats for the true late graft). Together, these results suggest that middle windows are more tied to which PT- or IT-like candidate is exposed, while upstream state and late computation must be modeled jointly to account for the measured final PT/IT next-token margin.
...
The key novelty is twofold. Methodologically, conditioning on the first natural PT/IT next-token disagreement converts paired-checkpoint diffing into a controlled token-level test. Prior work has focused on final behavior, extracted activation directions, or single-trajectory layerwise sharpening; those settings do not directly decompose a paired token-level disagreement. Empirically, this protocol shows that middle/upstream state and late depth windows in IT models are non-additive under depth-localized interventions: late readout is large only when conditioned on an IT-shaped upstream state.
...
To answer this, we combine four controls that are usually separate. First, because `KL(layer || own final)` is endpoint-relative, we use coarsened exact matching on final entropy, top-1 confidence, and top-1/top-2 margin to ask whether the late KL gap remains after balancing the most direct endpoint covariates. Second, we replay identical teacher-forced histories through paired checkpoints and compare same-layer JS without using either model's own final endpoint. Third, we run symmetric MLP grafts and swaps under matched histories, with matched random residual-projection controls as a specificity check for generic late-window sensitivity. Fourth, we intervene at first-divergence prefixes and measure both token identity and IT-vs-PT margin.

This protocol supports three claims:

1. **Headline result: late readout is context-gated by upstream state.** A residual-state x late-stack factorial measures non-additivity under intervention: the same IT late stack produces `+3.21` logits from IT upstream but only `+0.57` from PT upstream, a `5.6x` asymmetry with interaction `+2.64` logits (95% CI `[+2.54, +2.74]`). A factual/reasoning extension preserves the interaction (`+1.68`, 95% CI `[+1.59, +1.78]`) while flipping the late-only term negative from PT upstream, so the stable claim is joint upstream-late dependence rather than invariant positive late-only amplification.
...
To our knowledge, no prior paper has tested whether middle/upstream state and late depth windows in instruction-tuned models are non-additive under first-divergence interventions during next-token formation. The residual-state x late-stack factorial in Section 3.4 provides this measurement, and the positive interaction term (`+2.64` logits, 95% CI `[+2.54, +2.74]`) is the central empirical result of this paper.
...
The results build the context-gated late readout account in four internal steps, followed by a secondary behavioral bridge. Post-trained descendants show an observational delayed-stabilization signature; late MLP substitutions have the largest tested leverage on that signature under matched histories; first-divergence tests ask what aspect of next-token formation late windows affect; and a residual-state x late-stack factorial tests whether late readout depends on upstream state. The free-running behavioral follow-up is reported only as a consistency check on whether the same intervention family can move natural-decoding outputs in the predicted direction.
...
The convergence gap is therefore the observational signature that motivates the paper's intervention decomposition, not the window-level causal leverage claim by itself. Appendix A gives endpoint-matching, probe, commitment, per-family, and reverse-teacher robustness views.

### 3.2 Late MLPs have the largest tested leverage on delayed stabilization
...
### 3.5 Secondary behavioral consistency check
...
This evidence is deliberately secondary. A completed two-rater blind human audit agrees directionally on the four primary pairwise contrasts, but unresolved labels are common and rater agreement is heterogeneous: pairwise Cohen's kappa ranges from `0.01` to `0.52` across the four-label contrasts and from `0.00` to `0.48` after collapsing unresolved labels. These limits keep the behavioral result from carrying the mechanistic claim; Appendix B gives the judge design, Figure B1, human-audit tables, unresolved rates, and kappa caveats.

## 4. Related Work and Discussion

Late sharpening and late calibration are not new by themselves. The logit-lens and tuned-lens line makes late-layer prediction dynamics visible (nostalgebraist, 2020; Belrose et al., 2023), Lad et al. (2024) frames late computation as residual sharpening after earlier candidate construction, and Joshi et al. (2025) studies late confidence calibration after decision certainty has emerged. Our contribution is to test whether that late stage behaves autonomously under first-divergence interventions, and it does not under this readout: the same IT late stack produces a `+3.21` logit shift from IT upstream state versus `+0.57` from PT upstream state in the primary holdout, while a factual/reasoning extension preserves the interaction but flips the late-only PT-upstream term negative. The contribution is therefore not the existence of late sharpening or late calibration, nor an invariant positive late-only module, but the measurement that the late stage's effect is conditional on upstream state in these hybrid passes. Late post-training computation is functionally non-autonomous under this first-divergence measurement.
