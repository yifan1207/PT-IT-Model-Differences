# Context-Gated Late Readout in Instruction-Tuned Language Models
...
We show that late computation in instruction-tuned language models depends causally on upstream state; it is not an autonomous late instruction module. We demonstrate this at first-divergence prefixes: positions where a pretrained checkpoint (PT) and its post-trained descendant (IT), given identical history, first prefer different next tokens. In a residual-state x late-stack factorial across five dense PT/IT model families, the same IT late stack shifts the IT-vs-PT margin by `+3.21` logits from an IT-shaped upstream state but only `+0.57` logits from a PT upstream state, a `5.6x` asymmetry with positive interaction term `+2.64` logits. This means the same late computation behaves very differently depending on upstream state, violating an additive or modular interpretation of depth. To establish this, we combine endpoint-matched convergence trajectories, endpoint-free same-history replay, symmetric MLP grafts and swaps, matched random controls, and first-divergence token counterfactuals. The factorial result sits on a broader decomposition: middle-window substitutions transfer PT/IT token identity more than late substitutions (`26.0%` versus `17.6%` IT-token transfer in a PT host), while late MLP grafts and swaps have the largest tested leverage on an observational delayed-stabilization signature and matched random late perturbations are near zero (`+0.003` versus `+0.327` nats for the true late graft). Together, these results suggest that middle windows are more tied to which PT- or IT-like candidate is exposed, while late computation supplies context-gated readout that makes selected IT-like candidates win most strongly when the upstream state is already IT-shaped.
...
We call this position the **first-divergence prefix**: the exact token position at which a pretrained checkpoint and its post-trained descendant, given identical input history, first prefer different next tokens. This construct converts paired-checkpoint model diffing from aggregate behavior comparison into a controlled token-level disagreement: because the histories are still shared, depth-localized interventions can separate effects on token identity from effects on final margin without a generated-history confound.

By **context-gated late readout**, we mean that the effect of late computation on next-token margin depends strongly on the upstream residual state, rather than acting as a fixed additive module.

The key novelty is twofold. Methodologically, conditioning on the first natural PT/IT next-token disagreement converts paired-checkpoint diffing into a controlled token-level test. Prior work has focused on final behavior, extracted activation directions, or single-trajectory layerwise sharpening; those settings do not directly decompose the cause of a paired token-level disagreement. Empirically, this protocol reveals that middle/upstream state and late depth windows in IT models are causally non-independent: late readout is large only when conditioned on an IT-shaped upstream state.
...
1. **Headline result: late readout is context-gated by upstream state.** A residual-state x late-stack factorial shows causal non-independence: the same IT late stack produces `+3.21` logits from IT upstream but only `+0.57` from PT upstream, a `5.6x` asymmetry with interaction `+2.64` logits.
2. **Method: first-divergence prefixes make PT/IT disagreement experimentally local.** We intervene before either model has generated a different previous token, then measure token identity and final margin separately.
3. **Supporting decomposition: middle windows lean identity, late windows lean margin.** Middle-window substitutions transfer PT/IT token identity more than late substitutions, while late MLP substitutions and local write-out tests have the largest leverage on delayed stabilization and final-token margin.

To our knowledge, no prior paper has tested whether middle/upstream state and late depth windows in instruction-tuned models are causally non-independent during next-token formation. The residual-state x late-stack factorial in Section 3.4 provides this measurement, and the positive interaction term (`+2.64` logits, 95% CI `[+2.54, +2.73]`) is the central empirical result of this paper.

What we claim is a causal decomposition of PT/post-trained next-token formation at the tested depth-window granularity: middle windows are more tied to candidate identity, while late MLP and late-stack computation supply context-gated late readout for selected IT-like candidates. What we do not claim is a complete instruction-following circuit, a named middle-layer feature feeding a named late write-in component, or evidence that late layers autonomously implement assistant behavior.
...
Prior work gives nearby pieces: late prediction sharpening and confidence calibration, base-versus-post-training comparisons, instruction vectors, task-layer localization, behavioral steering directions, and mid-layer safety or preference localization. We do not treat any one of those ingredients as new. The distinction is their combination in one paired PT/IT model-diffing protocol: first-divergence prefixes, matched histories, symmetric depth-localized interventions, matched random falsifiers, identity/margin decomposition, and a residual-state x late-stack factorial at the exact prefix where paired checkpoints disagree.
...
The observational discovery metric motivating Section 3.1 is the layerwise convergence trajectory `KL(p_l || p_L)`, where `p_l` is the decoded distribution at layer `l` and `p_L` is the same model's final-layer distribution. This is a within-model stabilization metric: it asks how far the current layer is from the model's eventual next-token prediction. It is symmetric across PT and IT because each model is evaluated against its own endpoint, but it is deliberately endpoint-relative and does not carry the causal claim by itself.
...
The results build the context-gated late readout account in five steps: post-trained descendants show an observational delayed-stabilization signature; late MLP substitutions most strongly control that signature under matched histories; first-divergence tests ask what aspect of next-token formation late windows control; a residual-state x late-stack factorial tests whether late readout depends on upstream state; and free-running behavior changes in the predicted direction.

### 3.1 Motivation: IT models show delayed stabilization
...
The convergence gap is therefore the observational signature that motivates the paper's causal decomposition, not the causal claim by itself. Appendix A gives endpoint-matching, probe, commitment, per-family, and reverse-teacher robustness views.
...
![Figure 4: Candidate transfer and local late support. First-divergence tests show that middle substitutions transfer PT-vs-IT token identity more, while late IT MLP windows provide the strongest native IT-token support. The full late MLP update carries the IT-vs-PT margin write-in; the residual-opposing component is a geometric marker rather than the causal margin vector.](../results/paper_synthesis/exp20_exp21_handoff_synthesis.png)
...
### 3.4 Late readout depends on upstream state

The strongest causal test patches the full residual state at the late-window boundary and runs the downstream late stack/readout under a common PT or IT readout. Under the common IT readout, swapping in the IT late stack shifts the IT-vs-PT margin by `+0.572` logits (95% CI `[+0.495, +0.650]`) from a PT upstream state, but by `+3.207` logits (95% CI `[+3.091, +3.325]`) from an IT upstream state. The matched 2x2 decomposition gives a late-stack main effect of `+1.890` logits (95% CI `[+1.803, +1.969]`), a larger upstream-context effect of `+4.239` logits (95% CI `[+4.097, +4.379]`), and a positive interaction of `+2.635` logits (95% CI `[+2.540, +2.734]`). The common PT readout agrees (`+0.609`, 95% CI `[+0.541, +0.679]`, versus `+3.218`, 95% CI `[+3.101, +3.329]`, for IT-late given PT versus IT upstream).

This is the main non-independence result. If late layers implemented an autonomous instruction module, the IT late stack should produce a large IT-vs-PT margin even from a PT upstream state. Instead, the effect from PT upstream is much smaller, and the 2x2 interaction is strongly positive: late IT computation is important, but its effect is much larger when the state entering it is already IT-shaped. The pooled intervals are dense-family prompt-level bootstrap intervals; Appendix F gives the interaction details and scope cautions, and the claim is not that every family has the same magnitude or that the exact boundary is a sharp circuit boundary.

| Family | First-divergence records | IT late stack from PT upstream | IT late stack from IT upstream | IT/PT ratio | Interaction |
|---|---:|---:|---:|---:|---:|
| Gemma 3 4B | `600` | `+0.10` | `+6.18` | `60.5x` | `+6.08` |
| Llama 3.1 8B | `600` | `+0.79` | `+2.05` | `2.6x` | `+1.25` |
| Qwen 3 4B | `600` | `+0.59` | `+2.05` | `3.5x` | `+1.46` |
| Mistral 7B | `597` | `+0.61` | `+3.15` | `5.1x` | `+2.53` |
| OLMo 2 7B | `586` | `+0.76` | `+2.61` | `3.4x` | `+1.85` |
| Dense-5 | `2,983` | `+0.57` | `+3.21` | `5.6x` | `+2.64` |

The interaction is positive in all five dense families. Per-family rows are point estimates over available first-divergence records; the pooled Dense-5 row has the bootstrap intervals reported above. Gemma has the largest interaction (`+6.08`), but excluding Gemma leaves a four-family mean interaction of `+1.77` logits and a `3.57x` IT-upstream/PT-upstream ratio. Gemma's `60.5x` ratio reflects the unusually small PT-upstream effect (`+0.10` logits) in the denominator, so the interaction term is the primary non-independence measure for that family. The structural direction is cross-family, while magnitude is family-dependent.

A descriptive subgroup pass leaves the interaction positive in every reportable stratum of the primary Exp23 holdout slice. It is positive across the three prompt categories present in that slice (`GOV-CONV` `+2.05`, `GOV-FORMAT` `+3.61`, `SAFETY` `+2.83` logits), across collapsed IT-token categories (`CONTENT` `+2.50`, `FORMAT` `+2.60`, `FUNCTION_OTHER` `+2.81`), and for non-assistant-marker events (`+2.48`). A separate content/reasoning extension also preserves the central interaction (`+1.68` logits, 95% CI `[+1.59, +1.78]`), including factual prompts (`CONTENT-FACT` `+1.84`) and reasoning prompts (`CONTENT-REASON` `+1.34`). The decomposition is domain-dependent: in that extension, the IT late stack from PT upstream moves against the IT token (`-1.41` logits), so this result supports context-gated non-autonomy rather than an invariant positive late-module effect.

The interaction term is not negligible relative to the main effects. Under the common-IT readout, dividing the three positive coefficient magnitudes (`4.24 + 1.89 + 2.64 = 8.77` logits), the upstream-context main effect accounts for about `48%` of the positive coefficient-magnitude mass, the late-stack main effect for `22%`, and the upstream-by-late interaction for `30%`. This is a coefficient-magnitude scale check, not a variance decomposition; the qualitative point is that the interaction is comparable in magnitude to the late-stack main effect. The result does not rule out compatibility learned jointly by upstream and late computation; it rules out analyzing the IT late stack as an independent module whose effect is approximately unchanged across upstream states. Distinguishing "late computation actively reasons about upstream state" from "late computation is approximately a function of upstream state" requires feature-level mediation inside the upstream state, which we leave to future work (Section 5).

![Figure 5: Headline result: late computation is not autonomous. Residual-state x late-stack factorial at first-divergence prefixes. The same IT late stack shifts the IT-vs-PT margin by `+0.57` logits from a PT upstream state but `+3.21` logits from an IT upstream state, a `5.6x` asymmetry. Under this window-level measurement, the observed asymmetry rules out a simple additive/autonomous account. Under common-IT readout, the upstream main effect is `+4.24`, the late-stack main effect is `+1.89`, and the interaction is `+2.64` logits. The positive interaction quantifies the failure of additivity: upstream state and late computation must be analyzed jointly. Both common-PT and common-IT readouts agree.](../results/exp23_midlate_interaction_suite/exp23_dense5_full_h100x8_20260426_sh4_rw4/analysis/exp23_midlate_interaction.png)

The synthesis is the paper's headline. At the granularity tested here, middle windows are more diagnostic of which PT- or IT-like token candidate is exposed, while late MLP and late-stack windows help selected IT-like candidates dominate the final distribution.

The implication is non-modularity: the same late computation is positive yet much smaller from a PT upstream state, and strongest when the residual stream is already IT-shaped. Mid and late windows must be analyzed jointly; neither in isolation captures the PT/IT next-token disagreement.
...
Late sharpening and late calibration are not new by themselves. The logit-lens and tuned-lens line makes late-layer prediction dynamics visible (nostalgebraist, 2020; Belrose et al., 2023), Lad et al. (2024) frames late computation as residual sharpening after earlier candidate construction, and Joshi et al. (2025) studies late confidence calibration after decision certainty has emerged. Our contribution is to test whether that late stage is autonomous under post-training, and it is not: the same IT late stack produces a `+3.21` logit shift from IT upstream state versus `+0.57` from PT upstream state. The contribution is therefore not the existence of late sharpening or late calibration, but the demonstration that under post-training, the late stage's effect is conditional on upstream state. Late post-training computation is functionally non-autonomous under this first-divergence measurement.

The residual-opposing geometry supports this interpretation, but only as interpretation. Late IT MLP updates have a substantial component opposing the residual stream entering them, a geometry consistent with revision or reconciliation rather than simple additive accumulation. However, the residual-opposing component is not itself the causal margin write-in: it contributes `-0.0046` logits to the IT-vs-PT margin (95% CI `[-0.009, -0.001]`), while the full late MLP update contributes `+0.768` logits (95% CI `[+0.729, +0.805]`). We therefore do not claim that residual opposition is the causal source of late sharpening. The geometry is consistent with revision, but does not by itself write the IT-vs-PT margin.
...
The residual-state x late-stack factorial demonstrates non-independence but leaves the mechanism inside the upstream state unresolved. It shows that IT-shaped upstream residual state and IT late computation interact strongly on the first-divergence margin; it does not identify which upstream features, attention heads, or MLP directions carry that state. The next circuit-level test is therefore not another coarse window swap, but a feature- or subspace-level mediation test inside the middle-to-late handoff, which we leave to future work.
...
| Endpoint-free matched-prefix JS | same | `results/exp16_matched_prefix_js_gap/exp16_js_replay_runpod_20260422_075307/js_summary.json` | point estimates over `3,000` dense-family prompt records: `JS(A', C)=0.106` pre-corrective and `0.169` in the final 20%; bootstrap CI not materialized in this summary |
| Matched-prefix depth ablation | same | `results/exp11_matched_prefix_mlp_graft/plots/exp11_exp3_600rand_v11_depthablation_full/depth_ablation_metrics.json` | dense-family point estimates: early `-0.035`, middle `-0.045`, late `+0.341` nats |
| Symmetric late graft/swap | same | `results/exp14_symmetric_matched_prefix_causality/exp13exp14_full_20260416/exp13_full_summary.json` | dense-family point estimates: late IT-to-PT graft `+0.338`; late PT-to-IT swap `-0.509` nats |
...
| Residual-state x late-stack context gating | same | `results/exp23_midlate_interaction_suite/exp23_dense5_full_h100x8_20260426_sh4_rw4/analysis/exp23_summary.json`; `analysis/exp23_effects.csv` | common-IT late given PT upstream `+0.572` `[+0.495, +0.650]`; late given IT upstream `+3.207` `[+3.091, +3.325]`; upstream `+4.239` `[+4.097, +4.379]`; late-stack `+1.890` `[+1.803, +1.969]`; interaction `+2.635` `[+2.540, +2.734]` |
