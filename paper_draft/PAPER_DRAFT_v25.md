...
A terminal-depth audit sharpens what "late" means without replacing the main late-window result. On the 4B-8B Dense-5 support set, moving the factorial boundary to the final three transformer blocks preserves a positive interaction in every family: `+1.50` logits (`[+1.45, +1.56]`), or `57%` (`[56%, 58%]`) of the same-prompt full-late estimate. Excluding Gemma gives `+0.91` logits (`[+0.87, +0.95]`) and `51%` retention (`[50%, 53%]`). Moving the boundary to the final block alone remains positive but smaller: `+0.87` logits (`[+0.83, +0.91]`) and `33%` retention (`[32%, 34%]`) for Dense-5, or `+0.39` (`[+0.37, +0.42]`) and `22%` (`[21%, 23%]`) without Gemma. Thus the final blocks carry a robust terminal component of the interaction, especially the final three blocks, but the full late stack remains stronger. This boundary-retention ratio is a same-prompt depth-localization diagnostic, not an additive mediation fraction (Appendix A.10).
...
| Terminal-depth audit | Appendix A.10 | Dense-5 last-three and last-one boundary checks for the upstream × late interaction. |
...
| Terminal-depth boundary audit | `src/poc/exp23_midlate_interaction_suite/`; `scripts/run/run_exp31_terminal_depth_factorial_runpod.sh`; `scripts/analysis/analyze_exp31_terminal_depth_factorial.py` | `results/exp31_terminal_depth_factorial/exp31_terminal_depth_full_a100x4_localdisk_fixedsched_20260502_021238/analysis/` |
