...
A layerwise signature made this counterfactual worth testing: IT checkpoints show delayed stabilization, meaning decoded intermediate layers stay farther from the same checkpoint's final next-token distribution than PT layers do. Late-window graft/swap controls localize this delay to learned late MLP changes rather than generic late perturbations (§3.2). The first-divergence factorial asks the sharper model-diffing question: when PT and IT first disagree, does the learned IT late computation carry the IT-token margin by itself, or only in compatibility with an IT-shaped upstream state?
...
Grouped rows labeled Dense-6 include Qwen2.5-32B. Rows labeled Dense-5 or Dense-4 deliberately exclude it because the corresponding supporting analysis was only run or only pooled for the five 4B-8B families. The Qwen2.5-32B PT/IT run repeats the core first-divergence factorial, identity/margin, MLP write-out, and raw-KL bridge analyses; its standalone details are reported in Appendix A.7, and its interaction is included in the Dense-6 §3.1 estimate. A non-pooled OLMo-2 7B stage-progression case study applies the same first-divergence factorial to released Base, SFT, DPO, and Instruct/RLVR checkpoints; it is reported in §3.1 and Appendix A.8 as local transition evidence, not as an additive decomposition of the full Base→Instruct/RLVR contrast.
...
The practical diagnostic is the gap between two late-stack effects: the one-cell late-only patch from PT upstream, and the matched-context late effect from IT upstream. We call this the **late-effect portability gap** in the table below; algebraically it is the same upstream × late interaction, not a new estimand. The scope column matters: the Dense-6 row is the core family-balanced synthesis, while subgroup rows use the five-family artifacts that carry prompt, token, and position labels. The headline Dense-6 generated-position `≥3` interaction is `+1.43`; the `+1.52` row below is the corresponding Dense-5 labeled-position audit.

| Regime | Scope | Late-only from PT upstream | Matched-context late effect | Portability gap / interaction | Diagnostic read |
|---|---|---:|---:|---:|---|
| First-divergence | Dense-6 family-balanced core synthesis | `+0.64` | `+3.08` | `+2.44` | Late-only underestimates matched-context late computation. |
| Factual/reasoning extension | Dense-5 content/reasoning extension | `-1.18` | `+0.64` | `+1.81` | Sign failure for the late-only summary. |
| Later positions `≥3` | Dense-5 labeled-position audit | `+0.98` | `+2.49` | `+1.52` | Underestimates in later shared-history regimes. |
| Governance/conversation | Dense-5 subgroup-labeled records | `+0.64` | `+2.69` | `+2.05` | Underestimates. |
| Format/register | Dense-5 subgroup-labeled records | `+0.76` | `+4.37` | `+3.61` | Strong underestimation. |
| Safety | Dense-5 subgroup-labeled records | `+0.25` | `+3.08` | `+2.83` | Strong underestimation. |
...
All rows above use the common-IT 2×2 interaction readout. The diagnostic table separates the PT-upstream late-only term from the matched-context late effect; the robustness table below focuses on the interaction itself.
...
![Figure 4: Symmetric graft/swap localization. Left and center: late MLP substitutions have the largest convergence-gap effect in both directions, increasing the delay in a PT host and reducing it in an IT host. The matched random-control number in the text checks that this is not a generic late-window perturbation effect.](../results/exp14_symmetric_matched_prefix_causality/exp13exp14_full_20260416/exp13_full_causal_main.png)
...
| Auxiliary mechanism check | Appendix A.9 | Dense-5 controls that remove, scale, flip, PT-level, and random-orthogonalize one residual-opposing late-MLP component. |
...
### A.9 Auxiliary mechanism check: residual-opposition geometry

Exp26 tests whether one geometry-defined part of late MLP computation contributes to the first-divergence upstream × late interaction. It reruns the Dense-5 raw-shared residual-state × late-stack factorial at the same first-divergence prefixes as Exp23, but modifies only the component of late IT MLP updates that points opposite the incoming residual stream. This is not a complete feature-level circuit decomposition; it is a targeted support check for whether a residual-opposing late-revision geometry has causal leverage.
...
The effect is real but partial: removing one geometrically defined component accounts for roughly one-tenth of the Dense-5 interaction, while sign reversal accounts for roughly one-fifth. Most of the interaction remains in other late-stack computations and their compatibility with upstream state, so this appendix is a support factor rather than the main mechanism claim.

The paired PT-target control asks whether this is uniquely an IT-late phenomenon or a post-training change to a more generic late-finalization operation. Repeating the intervention on the analogous residual-opposing component inside the PT late stack gives substantial PT sensitivity too. Dense-5 averaging makes the IT target more sensitive, but family-median and Gemma-removed summaries show the opposite direction for the removal variant. The safe reading is therefore that residual opposition is functional in late stacks generally, and post-training reshapes or amplifies that operation in a family-dependent way.
