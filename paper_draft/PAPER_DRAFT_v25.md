...
**Selection baselines.** First divergence is intentionally a selected event, so Exp37 asks whether selection alone explains the interaction. It does not. Random local PT/IT disagreement prefixes also show an upstream x late interaction, but smaller than true first divergence: `+1.672` (`[+1.592, +1.751]`), about `63%` of the first-divergence value. In contrast, scoring the future first-divergence token pair on earlier shared prefixes where PT and IT had not yet diverged gives a near-zero interaction (`+0.082`, `[-0.058, +0.215]`, about `3%`). Thus first divergence is a high-signal disagreement regime, not an arbitrary token-pair artifact.
