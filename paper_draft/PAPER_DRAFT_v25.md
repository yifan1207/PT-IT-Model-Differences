...
Instruction-following descendants often choose different next tokens than their pretrained bases, but a late-layer patch alone can conflate the late computation with the upstream residual state it expects. We introduce **first-divergence model diffing**: for a paired base/instruct checkpoint, find the first shared-history prefix where the checkpoints prefer different next tokens, cross PT/IT upstream state with PT/IT late stack, and score the IT-vs-PT token margin. Using the conservative non-outlier summary (Gemma removed), the IT-minus-PT late-stack effect is `3.3x` larger from IT-shaped upstream state than from PT-shaped upstream state (`+2.46` vs `+0.75` logits), giving a `+1.71` logit interaction; including all six dense families gives `+2.44` logits (`4.8x`) because Gemma exhibits an amplified version of the same positive effect. The interaction remains positive at generated position `>=3`. The effect is not explained by arbitrary token-pair selection: random local disagreements retain only `63%` of the interaction, while pre-divergence prefixes scored on the future divergent token pair are near zero. Depth and feature analyses show the anatomy of the effect: middle MLP windows are more identity-selective, late/terminal windows are more margin-sensitive, and a fixed top-200 subset of causally ranked terminal crosscoder features accounts for a concentrated `26-48%` of the terminal readout interaction. The same terminal features are upstream-conditioned: ablating them hurts the IT-upstream terminal readout more than the PT-upstream readout, beating matched-random features by `+1.27` logits in four quality-gated families. In three non-outlier clean families, patching those terminal feature activations into the weak PT-upstream/IT-terminal hybrid partially rescues the missing margin by `+0.49` logits.
...
We then test partial sufficiency in the same feature basis. In three non-outlier clean families (Llama, Mistral, and Qwen), we patch top-200 terminal feature activations from the native `U_IT,L_IT` pass into the weak `U_PT,L_IT` hybrid. This rescues `+0.49` logits of IT-token margin (`[+0.45, +0.54]`) and beats matched-random rescue by `+0.56` logits (`[+0.51, +0.61]`) and same-delta random rescue by `+0.47` (`[+0.43, +0.52]`). This deliberately harder sufficiency check is not full reconstruction: it recovers about `8%` of the missing margin directly, and about `11%` over matched-random controls. It shows that the upstream-conditioned terminal features are not only necessary for the IT-upstream readout; moving their native-IT activation pattern into the PT-upstream hybrid causally recovers a measurable slice of the missing IT-token margin.
...
As a targeted semantic validation, we edit only the predeclared `structure_readout` bucket: `12` causally selected features labeled as response structure or answer-readout features across Gemma, Llama, Mistral, and Qwen. The `12` features are not the statistical sample size; they are a filtered, predeclared semantic subset whose falsifiable prediction is dose response against feature-matched controls. The edit is applied only inside each model's terminal crosscoder window. As the edit strength increases from `0` to `2`, the clean-family mean interaction drop increases monotonically (`0.000 -> 0.032 -> 0.066 -> 0.103 -> 0.149`). At the strongest paper-facing edit, all four families are positive (`+0.056`, `+0.091`, `+0.339`, `+0.110`), while matched-random and same-delta random controls are much smaller on average (`-0.050` and `+0.030`). We use this selectively: it supports the interpretation that response-structure/readout labels correspond to functional terminal readout handles; broader bucket steering remains exploratory.
...
The main claim is local to first-divergence next-token readouts. This is deliberate: the estimand measures the earliest token where a released PT/IT pair changes preference under shared history. This support targets decisions that base/instruct contrasts naturally expose, including instruction, safety, formatting, response-shaping, and content-token choices. Position-stratified and domain stress tests show that the interaction persists beyond the earliest generated positions, while its magnitude varies with prompt domain and generated position.

The main interventions are window-level compatibility tests. Hybrid-state validation makes practical artifact explanations unlikely, and terminal crosscoders provide concentrated feature-level mediation, upstream-conditioning, and feature-rescue checks, but the paper does not recover a full circuit. The core empirical scope is six dense PT/IT pairs. This means architecture generalization is deliberately limited: Mistral gives one sliding-window case, Gemma gives one hybrid local/global-attention case, and both have positive interactions, but one model per architectural variant is not enough to attribute magnitude to attention structure. Gemma is the clearest architectural clue: its interaction is much larger than the rest of the panel, and its late/readout effects are unusually strong. Mistral is also above the non-Gemma median, but far below Gemma. We therefore treat architecture as a plausible moderator of effect size, not an established mechanism. Supporting analyses remain Dense-5 where they were not rerun at 32B scale; DeepSeek-V2-Lite stays appendix-only because MoE routing and expert swaps require different controls. MoE generalization is consequently a real open question, not something implied by the dense-family result. The OLMo-2 result is a fixed-support case study of one released lineage, not a universal stage attribution.

The remaining natural next test is a stronger middle-to-terminal account: perturb or interpret middle-state features directly and measure whether they drive the upstream-conditioned terminal features. The feature-rescue result here gives partial sufficiency at the terminal readout, but not a complete explanation of how upstream state produces those terminal feature activations.
