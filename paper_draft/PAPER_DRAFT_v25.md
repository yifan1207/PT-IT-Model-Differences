# First-Divergence Factorial Diffing for Released Base-to-Instruct Model Pairs
...
Late-layer patching is a tempting one-cell summary of a released base-to-instruct model diff, but it can be unstable across prompt regimes. We show this at the first PT/IT next-token disagreement: on a factual/reasoning stress test, the late-only effect from PT upstream moves against the IT token (`-1.18` logits), while the upstream × late-stack interaction remains positive (`+1.81`). We introduce **first-divergence factorial diffing** to measure that interaction directly: at the earliest shared-history prefix where a released pretrained checkpoint (PT) and its released instruction-following descendant (IT) prefer different next tokens, we cross PT/IT upstream residual state with PT/IT downstream late stack and score the IT-vs-PT divergent-token margin. Across six dense PT/IT pairs, including Qwen2.5-32B, the interaction is positive in every family. We use the Gemma-removed Dense-5 estimate as the conservative magnitude headline: `+1.71` logits (95% CI `[+1.64, +1.78]`). Including Gemma, the Dense-6 family-balanced summary is `+2.44` logits (`[+2.35, +2.52]`). The effect is not confined to immediate response openings: restricting to generated positions `≥3` still gives `+1.43` logits (`[+1.30, +1.57]`), with all six family-specific intervals above zero. The sign is label-aligned under a permutation null on the 4B-8B five-family raw-record subset (`p=5×10⁻⁵`). A staged OLMo-2 case study shows where the same estimand appears along one released Base→SFT→DPO→RLVR path: the largest adjacent-stage jump is Base→SFT (`+0.78`), followed by smaller positive SFT→DPO (`+0.14`) and DPO→RLVR (`+0.016`) interactions. Supporting tests give a graded anatomy of the change: middle-positioned MLP substitutions transfer token identity more often, while late-positioned substitutions and native late MLP write-outs have larger margin/readout effects. The contribution is a reusable diagnostic protocol for paired-checkpoint diffing plus a cross-family characterization of how released instruction-following descendants reroute next-token formation relative to their pretrained ancestors.

---

## 1. Introduction

Suppose a released pretrained checkpoint (PT) and its released instruction-following descendant (IT) read the same prefix and arrive at different next-token preferences — say PT prefers `.` and IT prefers `,`. This is a model-diffing question: what changed inside the forward pass between the released base model and its instruction-following descendant? The practical risk is that a late-layer patch can be interpreted as a portable PT→IT mechanism when it is actually measuring compatibility between a late stack and the upstream residual state it expects. The point is not that late transformer blocks ordinarily ignore upstream state; they do not. The point is whether the released PT→IT change is portable as a late-only update, or whether the IT late computation becomes effective only after earlier layers have shaped the residual stream toward the IT candidate.
...
**Scope.** We use **IT** throughout as readable shorthand for released instruction-following descendants of released pretrained checkpoints. The recipes producing these IT checkpoints are heterogeneous — SFT, DPO, RLVR, preference optimization, and proprietary multi-stage post-training vary across families. The estimand is therefore the paired diff between a released PT checkpoint and its released instruction-following descendant, treating the recipe as part of the measured model pair. We do not attribute the effect to "instruction tuning" qua SFT or to any single recipe component.
...
| Term | Meaning |
|---|---|
| **PT / IT** | Released pretrained checkpoint and its released instruction-following descendant from the same model family. |
| **PT/IT pair** | One paired released pretrained/instruction-following checkpoint contrast. |
...
| Model | Layers | d_model | Attention | Pre-training data | Released descendant recipe |
|---|---|---|---|---|---|
...
A terminal-depth audit sharpens what "late" means without replacing the main late-window result. On the 4B-8B Dense-5 support set, moving the factorial boundary to the final three transformer blocks preserves a positive interaction in every family: `+1.50` logits (`[+1.45, +1.56]`), or `57%` (`[56%, 58%]`) of the same-prompt full-late estimate. Excluding Gemma gives `+0.91` logits (`[+0.87, +0.95]`) and `51%` retention (`[50%, 53%]`). Moving the boundary to the final block alone remains positive but smaller: `+0.87` logits (`[+0.83, +0.91]`) and `33%` retention (`[32%, 34%]`) for Dense-5, or `+0.39` (`[+0.37, +0.42]`) and `22%` (`[21%, 23%]`) without Gemma. Thus the final blocks carry a robust terminal component of the interaction, especially the final three blocks, but the full late stack remains stronger. The terminal-only MLP follow-up in §3.4 shows the same qualitative split as the broader depth analysis: terminal windows move margin/readout much more than they transfer token identity. This boundary-retention ratio is a same-prompt depth-localization diagnostic, not an additive mediation fraction (Appendix A.10).

The staged-lineage question is separated in §3.2 because it is not a pooled robustness check: it asks where this same estimand appears along one released Base→SFT→DPO→RLVR trajectory.
...
The Dense-6 result compares released PT and IT endpoints across heterogeneous recipes. OLMo-2 gives a cleaner within-lineage check because AllenAI released Base, SFT, DPO, and Instruct/RLVR checkpoints for the same 7B architecture and tokenizer family. We therefore rerun the first-divergence factorial on four OLMo transitions: Base→SFT, SFT→DPO, DPO→RLVR, and the full Base→RLVR endpoint contrast.

This case study asks a different question from the Dense-6 pool: not "is the interaction positive across families?", but "where does this interaction first appear along one released staged path?" Each row below uses its own first-divergence prefixes and divergent-token pairs, so the adjacent-stage estimates should not be summed to recover the full Base→RLVR number.
...
A terminal-only MLP follow-up makes this split sharper rather than replacing it. Under the raw-shared first-divergence protocol, final-three and final-one MLP substitutions transfer IT-token identity only weakly, while removing those terminal IT MLPs from an IT host produces a much larger margin drop. The terminal 2×2 margin interaction is positive for the final three MLP layers and remains positive, though smaller and more heterogeneous, for the final layer alone.

| Terminal readout (4B-8B Dense-5 support set, raw-shared) | Last 3 MLP layers | Last 1 MLP layer | What it says |
|---|---:|---:|---|
| PT host: IT-token identity transfer | `8.4%` `[7.5%, 9.5%]` | `6.1%` `[5.3%, 6.9%]` | Terminal layers rarely choose the IT token by themselves. |
| PT host: margin gain | `+0.083` `[+0.045, +0.125]` | `+0.055` `[+0.021, +0.091]` | Terminal insertion into PT upstream is small. |
| IT host: margin drop when terminal IT MLPs are replaced | `+1.151` `[+1.086, +1.216]` | `+0.640` `[+0.592, +0.689]` | Terminal MLPs matter much more in IT-shaped upstream states. |
| Terminal MLP margin interaction | `+1.068` `[+1.009, +1.127]` | `+0.584` `[+0.541, +0.630]` | Terminal MLPs carry a margin/readout component of the upstream-state interaction. |
| Local terminal MLP write-out interaction | `+0.087` `[+0.077, +0.097]` | `+0.110` `[+0.093, +0.126]` | The finite-difference write-out proxy has the same sign but is much smaller. |

This is the most defensible terminal-layer interpretation: final MLP layers participate in the readout/amplification side of the handoff, but they are not an independent identity selector. The geometry is suggestive but not yet a mediation result. In the same raw-shared records, the last-layer IT-vs-PT change in MLP `delta_cosine` is more negative in all five dense families, and Appendix A.9 shows that ablating late residual-opposing components hurts IT own-token prediction more than PT. However, the direct terminal residual-opposition margin component in this fixed-prefix proxy is compatible with zero, so we treat residual opposition as mechanism-adjacent context rather than the explanation of the terminal interaction.
...
The empirical unit is the released paired checkpoint diff. The IT checkpoints in the Dense-6 pool are instruction-following descendants produced by heterogeneous recipes, and we do not attempt to attribute the measured route change to SFT, preference optimization, RLVR, or proprietary recipe components separately. The OLMo-2 lineage gives one within-family stage case study of a released Base→SFT→DPO→RLVR path; it is not a general decomposition of all released IT recipes. Applying the same estimand to base↔continued-pretraining or base↔domain-adapted pairs is a separate follow-up that would test whether the pattern is specific to instruction-following descendants or appears whenever a descendant shifts the next-token distribution.

The model scope is six dense PT/IT pairs for the core first-divergence evidence: five 4B-8B families plus Qwen2.5-32B. Supporting layerwise, graft/swap, and random-control analyses remain Dense-5 where they were not rerun or pooled at 32B, and are labeled that way. We also include one non-pooled OLMo-2 7B stage-progression case study. This is broad for mechanistic model diffing, but it is not frontier-scale validation and it excludes non-dense claims from the main pool. The DeepSeek MoE run remains an appendix side case because dense MLP grafts and MoE routing perturbations are not the same intervention.

First-divergence events are selected token regimes, not random positions. That is the object of study: the protocol asks what happens at the first natural token where the released IT descendant changes the PT next-token preference. Position, prompt category, and token category stratification therefore characterize the effect rather than invalidate it. Native convergence-gap analyses are supporting context; the first-divergence factorial is the primary estimand.

## 6. Conclusion

In these released PT/IT pairs, the base-to-instruct diff does not appear as a portable late-only delta. At the first token where PT and IT disagree, IT late computation contributes much more IT-token margin when the upstream residual state is already IT-shaped; the same pattern is positive across six dense PT/IT pairs. In the OLMo-2 stage lineage, the largest adjacent-stage jump appears at Base→SFT, with smaller positive SFT→DPO and DPO→RLVR interactions and a large Base→RLVR contrast. The factual/reasoning stress test shows the practical reason to use the factorial: a simple late-only patch can reverse sign even when the upstream × late interaction remains positive. Middle-positioned substitutions move candidate identity more often, late-positioned substitutions move margin more, and terminal-only MLP grafts sharpen the anatomy: final blocks contribute to margin/readout much more than token identity, with the full late stack still stronger than the terminal subcomponent. First-divergence factorial diffing turns that handoff into an auditable model-diffing diagnostic: a way to measure how a released instruction-following descendant reroutes next-token formation relative to its pretrained ancestor.
...
The appendices are organized by purpose rather than by experiment ID. Appendix figures use local monotone labels (`Figure A1`-`A40` in Appendix A, `Figure B1` in Appendix B) rather than legacy run numbers; appendix tables use `Table A1`-`A7`. A reviewer checking a specific claim can use this map:
...
| Terminal-depth audit | Appendix A.10 | Dense-5 last-three and last-one boundary checks, plus terminal-only MLP identity/margin follow-up. |
...
Three auxiliary sections preserve extra experiments that are informative but not part of the paper's main evidence spine. Appendix A.9 asks whether the late residual-opposing MLP component matters during each model's own natural rollout. Appendix A.10 asks how much of the late-stack interaction remains when the boundary is moved to the final three or final one transformer blocks, then checks whether terminal-only MLP grafts look like identity selection or margin/readout. Appendix B asks whether the same intervention family has visible consequences under free-running natural decoding. These are useful sanity and mechanism-adjacent checks; none is used to establish the core first-divergence factorial claim.
...
[Figure A6: Feature importance analysis. Per-feature contribution to the late PT→IT computation difference at layers 20–33, showing the distribution of importance across transcoder features.](../results/exp03_corrective_stage_characterization/plots/plot_e3_11_feature_importance.png)
...
The OLMo-2 stage-progression estimates are reported in §3.2. Exp25 uses the released OLMo-2 7B Base, SFT, DPO, and Instruct/RLVR checkpoints to ask whether the upstream × late interaction appears in adjacent released-stage transitions, not only in the full Base→Instruct/RLVR contrast. This is a single-lineage case study and is not pooled with the Dense-6 core estimate. Each adjacent transition has its own first-divergence events and divergent token pair, so the adjacent estimates are local transition measurements rather than additive components of the full Base→RLVR number.
...
The strongest defensible conclusion is absolute importance, especially for direct own-token logit support. We do not claim from this table that IT has universally larger per-unit NLL sensitivity to residual opposition, nor that residual opposition alone mediates the first-divergence upstream × late interaction. Instead, Exp27 supports the auxiliary mechanism picture: in these released pairs, the late residual-opposing component is much more consequential for the IT model's own next-token readout, while the main paper's causal estimand remains the first-divergence upstream × late interaction.

### A.10 Terminal-depth boundary audit

Exp31 asks how much of the §3.1 upstream × late-stack interaction remains if the residual-state factorial boundary is moved from the pre-specified late-window start to the final transformer blocks. This is a full downstream-stack boundary audit: at boundary `last3`, the upstream state already includes all but the final three transformer blocks; at boundary `last1`, it includes all but the final block. The retention ratio is therefore a same-prompt depth-localization diagnostic, not an additive attribution or mediation fraction.

The run covers the 4B-8B Dense-5 support set under the same raw-shared first-divergence design as §3.1. All expected boundary records are present (`5` families × `2` boundary modes × up to `600` prompts), with zero invalid events. The full-late reference is the existing Dense-5 Exp23 run, restricted to overlapping prompt clusters for each ratio. Common-PT readouts give the same conclusion; the table reports common-IT for compactness.

**Table A6: terminal-depth boundary audit.** Interactions are upstream × downstream-stack effects on the divergent-token margin. Retention is the boundary interaction divided by the same-prompt full-late interaction.

| Family or summary | Last-3 interaction | Last-3 retention | Last-1 interaction | Last-1 retention |
|---|---:|---:|---:|---:|
| Gemma 3 4B | `+3.87` `[+3.66, +4.08]` | `64%` `[62%, 65%]` | `+2.77` `[+2.60, +2.94]` | `46%` `[44%, 47%]` |
| Llama 3.1 8B | `+0.54` `[+0.45, +0.63]` | `43%` `[39%, 47%]` | `+0.25` `[+0.21, +0.30]` | `20%` `[17%, 23%]` |
| Qwen 3 4B | `+1.02` `[+0.94, +1.11]` | `70%` `[65%, 75%]` | `+0.43` `[+0.36, +0.49]` | `29%` `[25%, 33%]` |
| Mistral 7B | `+1.22` `[+1.14, +1.31]` | `48%` `[47%, 50%]` | `+0.52` `[+0.48, +0.57]` | `21%` `[20%, 22%]` |
| OLMo 2 7B | `+0.85` `[+0.77, +0.93]` | `46%` `[44%, 49%]` | `+0.37` `[+0.33, +0.41]` | `20%` `[18%, 22%]` |
| Gemma-removed Dense-4 | `+0.91` `[+0.87, +0.95]` | `51%` `[50%, 53%]` | `+0.39` `[+0.37, +0.42]` | `22%` `[21%, 23%]` |
| Dense-5 family mean | `+1.50` `[+1.45, +1.56]` | `57%` `[56%, 58%]` | `+0.87` `[+0.83, +0.91]` | `33%` `[32%, 34%]` |
| Trimmed mean | `+1.03` `[+0.98, +1.08]` | `53%` `[52%, 54%]` | `+0.44` `[+0.41, +0.47]` | `23%` `[21%, 24%]` |

![Figure A40: Terminal-depth boundary audit. Moving the residual-state factorial boundary to the final three blocks preserves a substantial positive interaction across Dense-5 families, while the final block alone remains positive but smaller.](../results/exp31_terminal_depth_factorial/exp31_terminal_depth_full_a100x4_localdisk_fixedsched_20260502_021238/analysis/terminal_depth_retention.png)

Two details matter for interpretation. First, every Dense-5 support family has a positive last-three and last-one interaction interval, so the terminal component is not a Llama-only artifact. Second, Gemma is again high magnitude: excluding it lowers last-one retention from `33%` to `22%`, while last-three retention remains substantial (`51%`). The paper-safe conclusion is therefore that the final blocks carry a robust terminal component of the upstream × late interaction, especially the final three blocks, but the effect is not last-layer-only and the full late stack remains the stronger intervention.

Position-stratified audits tell the same qualitative story. In the Gemma-removed Dense-4 subset, last-three remains positive at generated position `>=3` (`+0.51` logits, 95% CI `[+0.46, +0.56]`) and `>=5` (`+0.55`, `[+0.49, +0.61]`); last-one also remains positive but smaller (`+0.23`, `[+0.19, +0.26]` at `>=3`; `+0.25`, `[+0.20, +0.29]` at `>=5`). We do not emphasize per-family position-retention ratios because the full-late denominator can be small in some later-position strata.

Exp32/Exp33 ask whether this terminal-stack signal looks like token-identity selection or terminal MLP margin/readout. The answer is mostly the latter. In raw-shared terminal-only MLP grafts, PT-host IT-token identity transfer is low (`8.4%` `[7.5%, 9.5%]` for last-three; `6.1%` `[5.3%, 6.9%]` for last-one), far below the middle-window identity-transfer rate reported in §3.4. But the same terminal MLP replacement produces a positive IT-host margin drop (`+1.151` `[+1.086, +1.216]` for last-three; `+0.640` `[+0.592, +0.689]` for last-one). The induced terminal MLP margin interaction is therefore positive in aggregate and, for last-three, positive in every family.

**Table A7: terminal-only MLP margin interaction.** Rows use the 4B-8B Dense-5 raw-shared first-divergence support set. The outcome is the IT-vs-PT divergent-token margin. Last-one is aggregate-positive but more heterogeneous; Llama's last-one interval is compatible with zero.

| Family or summary | Last-3 MLP margin interaction | Last-1 MLP margin interaction |
|---|---:|---:|
| Gemma 3 4B | `+3.230` `[+3.056, +3.418]` | `+2.324` `[+2.192, +2.469]` |
| Llama 3.1 8B | `+0.230` `[+0.144, +0.314]` | `-0.032` `[-0.071, +0.008]` |
| Qwen 3 4B | `+0.569` `[+0.501, +0.636]` | `+0.155` `[+0.109, +0.201]` |
| Mistral 7B | `+0.789` `[+0.737, +0.844]` | `+0.221` `[+0.201, +0.241]` |
| OLMo 2 7B | `+0.508` `[+0.444, +0.568]` | `+0.245` `[+0.214, +0.273]` |
| Gemma-removed Dense-4 | `+0.524` `[+0.491, +0.558]` | `+0.146` `[+0.128, +0.165]` |
| Dense-5 family mean | `+1.068` `[+1.009, +1.127]` | `+0.584` `[+0.541, +0.630]` |

The corresponding local finite-difference MLP write-out proxy has the same sign but is much smaller (`+0.087` `[+0.077, +0.097]` for last-three; `+0.110` `[+0.093, +0.126]` for last-one). This scale difference is why the terminal result is framed as a margin/readout participation result, not as evidence that the final MLP update alone mediates the Exp31 downstream-stack interaction. Residual-opposing geometry is a plausible clue: in these records the last-layer IT-vs-PT change in MLP `delta_cosine` is negative in all five families, and Appendix A.9 shows that removing late residual-opposing components hurts IT own-token prediction. But the direct terminal residual-opposition margin term in Exp32 is near zero, so this remains a follow-up hypothesis rather than a mechanism claim.
...
| Terminal-depth boundary audit | `MODE=full bash scripts/run/run_exp31_terminal_depth_factorial_runpod.sh`; analysis via `python scripts/analysis/analyze_exp31_terminal_depth_factorial.py` | `results/exp31_terminal_depth_factorial/exp31_terminal_depth_full_a100x4_localdisk_fixedsched_20260502_021238/analysis/terminal_depth_summary.json`; `terminal_depth_effects.csv`; `terminal_depth_retention.png` | last-three Dense-5 interaction `+1.50` `[+1.45, +1.56]`, retention `57%` `[56%, 58%]`; last-one Dense-5 `+0.87` `[+0.83, +0.91]`, retention `33%` `[32%, 34%]`; Gemma-removed last-three `+0.91` `[+0.87, +0.95]`, retention `51%` `[50%, 53%]`; Gemma-removed last-one `+0.39` `[+0.37, +0.42]`, retention `22%` `[21%, 23%]` |
| Terminal MLP write-out audit | `bash scripts/run/run_exp32_terminal_mlp_writeout_runpod.sh`; analysis via `python scripts/analysis/analyze_exp32_terminal_mlp_writeout.py` | `results/exp32_terminal_mlp_writeout/exp32_terminal_mlp_full_dense5_a100x8_w2_20260502_043950/analysis/exp32_terminal_mlp_summary.json`; `exp32_terminal_mlp_effects.csv`; `exp32_terminal_mlp_writeout.png` | local terminal MLP margin interaction: last-three `+0.0867` `[+0.0766, +0.0968]`; last-one `+0.1099` `[+0.0928, +0.1260]`; direct terminal residual-opposition margin component compatible with zero |
| Terminal identity/margin audit | `bash scripts/run/run_exp33_terminal_identity_margin_runpod.sh`; analysis via `python scripts/analysis/analyze_exp33_terminal_identity_margin.py` | `results/exp33_terminal_identity_margin/exp33_terminal_identity_margin_full_dense5_a100x8_overlap_20260502_0509/analysis/exp33_terminal_identity_margin_summary.json`; `exp33_terminal_identity_margin_effects.csv`; `exp33_terminal_identity_margin.png` | PT-host IT-token transfer: last-three `8.4%` `[7.5%, 9.5%]`, last-one `6.1%` `[5.3%, 6.9%]`; terminal MLP margin interaction: last-three `+1.068` `[+1.009, +1.127]`, last-one `+0.584` `[+0.541, +0.630]` |
...
| Terminal-depth and terminal-MLP audits | `src/poc/exp23_midlate_interaction_suite/`; `src/poc/exp20_divergence_token_counterfactual/`; `src/poc/exp21_productive_opposition/`; `scripts/run/run_exp31_terminal_depth_factorial_runpod.sh`; `scripts/run/run_exp32_terminal_mlp_writeout_runpod.sh`; `scripts/run/run_exp33_terminal_identity_margin_runpod.sh`; terminal analysis scripts under `scripts/analysis/` | `results/exp31_terminal_depth_factorial/exp31_terminal_depth_full_a100x4_localdisk_fixedsched_20260502_021238/analysis/`; `results/exp32_terminal_mlp_writeout/exp32_terminal_mlp_full_dense5_a100x8_w2_20260502_043950/analysis/`; `results/exp33_terminal_identity_margin/exp33_terminal_identity_margin_full_dense5_a100x8_overlap_20260502_0509/analysis/` |
