...
Late layers sit closest to the unembedding, making them a natural place to localize base-to-instruct changes in next-token preference. But localization alone does not test portability: if an instruct model's late stack runs on the base model's upstream residual state, does the instruct preference still appear? We introduce first-divergence cross-patching, which crosses PT/IT upstream states with PT/IT late stacks at the first token where paired checkpoints disagree and measures the IT-token margin. Across five dense families ($4\mathrm{B}$–$32\mathrm{B}$), the IT late stack is partially portable but mostly upstream-conditioned: it adds $+0.76$ logits from base upstream and $+2.44$ from IT upstream, a $+1.68$ logit interaction that is positive in every family. The pattern survives controls for hybrid-state validity, token selection, and pre-late commitment. It is also recipe-structured: same-base instruction-tuned descendants show it, while a math-domain fine-tune (OpenMath2) has largely portable late effects. Mechanistically, sparse terminal crosscoder features mediate $26$–$48$% of the interaction, and a low-rank boundary-state shift recovers $71$% of the missing margin. For model diffing, patched-layer effects should not be assumed portable without testing dependence on prior residual state.
...
The tempting answer is "late layers." Late transformer computation is close to the unembedding, and prior work makes late-stage refinement plausible. But ordinary late-layer patching leaves a crucial ambiguity. A late stack can look causal partly because it is paired with the upstream residual state it was trained to read, not because its effect would transfer unchanged as a late-only update.

We do not exhaustively map every computation changed by post-training. Instead, we test a diagnostic implication of a late-only account: if instruction-tuned preferences were implemented as mostly portable late readouts, the IT late stack should retain most of its effect on PT upstream state.

We test portability directly with **first-divergence cross-patching**. For each prompt, we find the earliest generated position where PT and IT prefer different next tokens under the same generated history. Let those tokens be `t_PT` and `t_IT`. At that prefix, cross-patching pairs one checkpoint's upstream residual state (`U_PT` or `U_IT`) with one checkpoint's downstream late stack (`L_PT` or `L_IT`) and measures `logit(t_IT) - logit(t_PT)`. The key estimand is a difference-in-differences: how much larger is the IT-minus-PT late-stack replacement effect when the upstream state is IT-shaped rather than PT-shaped?
...
> - Interaction: `+1.68` logits; matched/portable ratio: `3.2x`.
...
1. **A paired-checkpoint first-divergence cross-patching factorial.** We introduce a local counterfactual estimand at the exact token where PT and IT first disagree. It decomposes the IT-minus-PT late-stack replacement effect into a portable component that transfers to PT upstream and an upstream-conditioned component that appears under IT-shaped upstream state.
2. **A validation and specificity ladder for the estimand.** We test the practical artifact explanations: broken hybrids, arbitrary selected-token support, pre-late commitment, readout choice, label orientation, position support, family heterogeneity, whether this is an automatic consequence of fine-tuning, and whether released stage lineages show a coherent profile. The main text gives the decisive checks; the appendices give provenance.
3. **A sparse terminal-feature bridge.** Terminal crosscoders connect the window-level result to sparse MLP features that partially mediate the readout interaction, matter more under IT-shaped upstream state, partially rescue the weak hybrid, and respond to preterminal computation patches. Depth anatomy motivates these terminal feature tests, and a separate structured boundary-state closure test provides an independent check in §3.4. This is a partial bridge, not full circuit reconstruction.

Scope is intentionally local. In the Core-5 base/instruct contrasts, **PT** denotes the released pretrained or base checkpoint and **IT** denotes the released instruction-following descendant. For same-base recipe and stage-lineage checks, we name the descendant or stage explicitly (for example, Meta Instruct, Tulu SFT, Tulu DPO, Tulu Final/RLVR, and OLMo RLVR/Instruct) rather than treating all stages as one IT checkpoint. The empirical claim is about released dense base/instruct checkpoint contrasts and related released descendant checks, not one training algorithm. Causal language is readout-scoped: replacing an upstream state, late stack, or MLP component changes a specified next-token readout in a constructed forward pass. We do not claim a universal instruction-following circuit or full circuit reconstruction.
...
The main factorial uses five dense PT/IT pairs: Llama 3.1 8B, Qwen 3 4B, Mistral 7B, OLMo 2 7B, and Qwen2.5 32B. We call this the **Core-5 set**. Several support analyses were run before the 32B extension and use the four smaller families: Llama, Qwen 3, Mistral, and OLMo. We call this the **Core-small support set**. Appendix A gives the full reproducibility snapshot, including fixed checkpoint revisions, prompt support, valid-event counts, first-divergence position mix, and late-stack boundaries.

Prompt-bootstrap intervals quantify conditional precision on the sampled prompts and released checkpoints. They are not estimates over all possible model families, post-training recipes, or prompt distributions. For headline family-generalization claims, we therefore report the Core-5 mean and family-level range or median where it helps interpretation. The Appendix Roadmap after the references maps each main claim to its audit trail and artifact roots.
...
Larger `Y` means the hybrid forward pass favors the IT divergent token. Late-stack boundaries were fixed before the Core-5 synthesis using a consistent architecture-relative rule: the late stack starts at roughly 60% depth and includes the final blocks, for example layers `19-31` for Llama 3.1 8B. Appendix A gives full per-family boundaries. We then run the four cells below:
...
> **What this estimand measures.** First-divergence cross-patching is a local next-token readout test. It does not estimate average instruction-following behavior or claim that hybrid states are natural trajectories. It tests whether an IT late-stack replacement effect is portable across PT and IT upstream residual states at natural PT/IT disagreement prefixes.
...
**Figure 1: First-divergence schematic and token examples.** Panel A shows the four hybrid passes used to estimate the upstream x late interaction. Panel B gives illustrative divergent-token pairs; all quantitative claims use the full support in Appendix A.
...
| Scope/readout | Late effect from PT upstream | Late effect from IT upstream | Portable share | Interaction | Matched/portable ratio |
|---|---:|---:|---:|---:|---:|
...
Likewise, the `31.1%` portable share is local to this divergent-token margin. It does not imply that `31.1%` of instruction-following behavior transfers as a late-only update during deployment.
...
The cross-patching factorial intentionally selects a high-signal disagreement point and constructs hybrid forward passes. No patching experiment makes those hybrids literally natural trajectories. The validation question is narrower: do the main artifact explanations make the estimand uninformative? The decisive checks pass; Appendix C gives the full audit trail.
...
- **Pre-late commitment does not explain the effect.** The interaction remains positive when the IT boundary readout does not yet favor `t_IT` and in the lowest boundary-margin tercile.
...
Secondary checks agree: later-position subsets remain positive, and every core family has a positive interaction. Together, these controls make the practical artifact explanations unlikely. The exact logit magnitude remains intervention-scoped, but the upstream-conditioned late-readout pattern is robust under the tests that would otherwise explain it away.

### 3.3 Recipe Structure and Continuation Persistence

The Core-5 result could still be read as an automatic native-state compatibility effect for any fine-tuned descendant, or as a one-token artifact. The main checks here ask whether the profile depends on the descendant recipe and whether it persists when the first divergent token is extended into a short constrained continuation. Released lineages provide a supporting fixed-support sanity check; blind judge details are appendix-only.

| Check | Main result | Read |
|---|---:|---|
| Same-base recipe control | instruction-following descendants exceed OpenMath2 by `+1.335` logits after matching; OpenMath2 is near zero on math support (`-0.154`) despite large late effects | the split is recipe-structured, not automatic fine-tune compatibility |
| Constrained continuation | instruction-following interaction grows from `+1.50` at `N=0` to `+2.71` at `N=8`; coherent-tail control is `+2.46` vs `+0.87` shuffled | the split persists beyond the selected token under fixed candidate continuations |
| Released stage lineages | Tulu-3 and OLMo-2 show partial SFT presence; DPO/preference checkpoints reach `84-85%` of final-support score | not a one-off final-checkpoint accident, but not causal stage attribution |

The same-base recipe control is the most direct anti-tautology check. On descendants of Llama-3.1-8B, general-purpose instruction-following checkpoints show upstream-conditioned late readout on instruction/format support, while OpenMath2, a math-domain SFT of the same base, does not reproduce that profile. The OpenMath result is informative rather than merely weak: on math-domain support its matched-context late effect is large, but the late effect is already portable from base upstream. In these supports, instruction-following descendants look co-adapted across upstream and late computation, while the domain-specialized math fine-tune looks more like a portable late computation. Appendix G gives the per-descendant table, math-domain control, and sign-flip null.

The constrained-continuation bridge keeps the test close to the factorial. After forcing the descendant-preferred divergent token, we teacher-force short native descendant/base continuations through the same four cells and score sequence log probability. For general-purpose instruction-following descendants, the interaction remains positive and grows over `N=8`; coherent descendant tails carry much more interaction than shuffled tails. OpenMath again behaves differently and is readout-sensitive at longer horizons, so we use this bridge for the instruction-following persistence claim rather than as a universal sequence-level recipe taxonomy.

Finally, released stage sweeps on Tulu-3 and OLMo-2 show that the measured interaction is already visible at SFT and that the DPO/preference checkpoint expresses most of the final fixed-support score. These are cumulative checkpoint comparisons, not causal attributions to SFT, DPO, or RLVR algorithms. Appendix G also reports a blind short-completion judge audit as a descriptive check: it confirms detectable intervention contrasts, but the broad free-running behavioral interaction is mixed and not load-bearing.

### 3.4 Mechanistic Bridge: Sparse Terminal Features and Structured State

The following analyses are not required for the main portability result; they ask whether part of the readout interaction can be traced to terminal MLP features and structured boundary-state shifts.

> **Bridge tested:** `preterminal computation -> terminal sparse features -> IT-token margin`.

Window anatomy (§3.4.1) explains why terminal MLPs are a good target. Sparse terminal features (§3.4.2-§3.4.3) are the main feature-level bridge: they mediate part of the interaction, matter more under IT upstream, and produce or track partial rescues. The boundary-state closure test (§3.4.4) is methodologically separate: it asks whether the missing upstream residual state has low-rank structure even without crosscoder features. This is a partial bridge, not full circuit reconstruction.
...
| Link tested | Evidence | Main result | Read |
|---|---|---:|---|
| Depth anatomy | middle-vs-late identity and margin transfer | middle is more identity-selective; late MLPs carry most native margin | motivates terminal feature tests |
| Terminal features -> margin | ablate top causally ranked terminal crosscoder features | `26-48%` mediated vs near-zero/wrong-sign random controls | sparse terminal features carry a concentrated part |
| Upstream -> terminal features | feature rescue plus mid/preterminal patch and reverse patch | `+0.49` rescue; `+1.71` upstream patch with `+0.13` mediated; `+3.57` reverse drop with `+0.53` mediated | features are upstream-conditioned and partly driven by preterminal state |
| Structured state closure | PCA patches of descendant-minus-base boundary state | rank-256 closes `0.71` of missing margin | independent residual-state check, not a crosscoder result |

#### 3.4.1 Window Anatomy

The window-level anatomy is a candidate-to-margin handoff: middle windows transfer divergent-token identity, meaning which token wins, more often, while late and terminal windows are more margin-sensitive, meaning how strongly that token wins. Middle-positioned MLP substitutions transfer divergent-token identity more often than late substitutions, while late windows dominate native IT-token support. Terminal-depth audits in Appendix D sharpen the same story: the last few blocks preserve a large readout subcomponent but transfer token identity poorly. The handoff is therefore operational rather than a complete circuit.

#### 3.4.2 Sparse Terminal Features Carry a Concentrated Part

Terminal crosscoders connect the window-level handoff to sparse MLP features. We train paired PT/IT BatchTopK crosscoders with a shared latent dictionary and separate PT/IT decoder branches on terminal MLP outputs, rank features by held-out causal effect, and ablate their IT-branch decoder contribution inside the terminal IT stack. The fixed top-200 causal subset accounts for `26-48%` of the terminal readout interaction, while matched-random sets have the wrong sign or near-zero effect. Appendix E gives reconstruction, sparsity, and training details.

For the feature edits below, rescue is reported as an absolute logit gain; rescue fractions in Appendix E divide by the missing margin defined above.
...
The same sparse terminal features inherit the upstream conditioning seen at window level. Ablating the top-200 causal terminal features hurts the `U_IT,L_IT` readout much more than the `U_PT,L_IT` readout, and patching their native `U_IT,L_IT` activations into the weak `U_PT,L_IT` hybrid gives a `+0.49` logit rescue. Both effects beat matched-random controls. This feature rescue is deliberately harder than ablation and is not full reconstruction, but it shows that the upstream-conditioned terminal features recover a measurable slice of the missing IT-token margin.

Finally, a direct handoff test perturbs upstream computation and re-measures the same terminal features. Injecting IT mid-to-preterminal computation into the weak `U_PT,L_IT` hybrid rescues `+1.71` logits of IT-token margin, with `+0.13` logits mediated by the selected terminal features. The reverse intervention, replacing the same preterminal computation in native IT with PT computation, causes a `+3.57` logit drop, with `+0.53` mediated. A terminal-entry patch gives the expected larger upper-bound sanity check (Appendix E). Thus the feature bridge is not only terminal: preterminal state changes drive a measurable part of the terminal sparse-feature readout.

#### 3.4.4 Independent Structured Boundary-State Closure

This subsection is not a crosscoder result. It is an independent state-space check of the same missing-upstream story. On Llama-3.1 descendants, we fit PCA directions to train-split descendant-minus-base boundary-state shifts and inject held-out projections into the weak base-upstream/descendant-late hybrid. At terminal boundary 31, a rank-256 projection closes `0.71` of the missing IT-token margin; the full held-out delta gives the expected upper bound (`0.97`). Matched Gaussian and random full directions are near zero, and sign-flipped directions go negative. We use this as structured residual-state support for upstream-conditioned readout, not as a recipe-level or completion-level claim.

Held-out autointerp and the predeclared `structure_readout` edit make the mediated feature set more readable, but they are not load-bearing causal evidence. Appendix E reports the validation AUROC, dose response, and controls.

### 3.5 What This Measures

The first-divergence event is a distributional fact: released PT and IT checkpoints prefer different next tokens at the same prefix. The four-cell test asks the mechanistic readout question: once the token contrast is fixed, how much of the IT late-stack effect ports across upstream states? The answer is some, but not most. Random local disagreements, pre-divergence future-token scoring, and pre-late commitment controls make the main artifact explanations unlikely, while the feature bridge shows that part of the interaction is carried by terminal sparse features that are more causally important under IT-shaped upstream state and respond to upstream/preterminal patches.

A supporting late-signature analysis stays in the appendix. Endpoint-matched late-refinement signatures explain why late/terminal windows were a good target, but they are not the mechanism itself (Appendix F). Contemporaneous token-level RLVR work is complementary: Sparse but Critical studies how token substitutions propagate during sampling (Meng et al., 2026), while our question is how the selected IT-token preference becomes a logit inside paired checkpoints.

---

## 4. Related Work

**Late refinement and post-training diffs.** Feed-forward layers promote vocabulary-space concepts and refine predictions (Geva et al., 2022a,b), and layerwise/tuned-lens analyses describe late residual sharpening and confidence adjustment (nostalgebraist, 2020; Belrose et al., 2023; Lad et al., 2025; Joshi et al., 2025). Post-training diff work studies behavioral and mechanistic shifts from base to aligned or instruction-following checkpoints (Wu et al., 2024; Zhao, Ziser, and Cohen, 2024; Du et al., 2025; Chaudhury, 2025). Sparse but Critical analyzes RLVR as sparse token-level distribution shifts whose cross-sampled substitutions affect reasoning trajectories (Meng et al., 2026). Our question is complementary and internal: at the first token where paired PT/IT checkpoints disagree, how much of the IT-token margin comes from a portable late-stack effect, and how much requires IT-shaped upstream state?
...
**Novelty.** The qualitative possibility of upstream/late co-adaptation is compatible with prior fine-tuning and transferability results. The contribution is to turn that possibility into a measured estimand: the portable-vs-upstream-conditioned split in the IT-minus-PT late-stack replacement effect at a natural disagreement token. The feature contribution is a partial bridge from this window-level interaction to terminal sparse features that mediate the readout, matter more under IT-shaped upstream state, partially rescue the weak hybrid, and are partly driven by preterminal computation.
...
Released base-to-instruct transitions appear to reshape how existing computation is read out, rather than adding a fully portable late instruction module. This fits a broader view of post-training as selecting or consolidating reachable behaviors from the pretrained model, but our evidence is narrower: dense checkpoint pairs, first-divergence token readouts, and constructed cross-patches.

The same-base recipe check sharpens the interpretation. OpenMath2 is a strong math-specialized SFT of Llama-3.1-8B, and on math-domain support its late effect is large but already portable from base upstream. General-purpose instruction-following descendants remain upstream-conditioned on the same support. Within this comparison, the factorial distinguishes mostly portable domain computation from co-adapted upstream-to-late readout. The Tulu-3 and OLMo-2 sweeps show that the same estimand can be reused along released lineages: in both, the interaction is partly present after SFT, the DPO/preference checkpoint expresses most of the final-support score, and the final checkpoint is strongest. These are cumulative checkpoint comparisons, not universal training-stage attributions.

### 5.2 Scope

The estimand is local to first-divergence next-token readouts. It targets the earliest point where a released PT/IT pair changes preference under shared history, not an average over deployment behavior. The interventions are window-level compatibility tests: validation makes practical hybrid artifacts unlikely, constrained continuation shows short-horizon likelihood persistence rather than free-running behavior, and crosscoders expose a partial, terminal, quality-gated sparse-feature trace. The headline logit margins should not be read as direct estimates of completion-level behavior.

The empirical scope is five dense core PT/IT pairs, one same-base recipe control, two released dense lineages, and one dense 32B scale check. DeepSeek-V2-Lite is appendix-only because MoE routing and expert swaps need different controls. Architecture and MoE generalization remain next-step questions.

### 5.3 Practical Implications and Next Tests

The practical implication is methodological: late-patching, steering, and model surgery studies on paired checkpoints should test whether late effects are portable across upstream states before treating them as standalone late-layer mechanisms. The next mechanistic step is to make the upstream side sparse as well: train or interpret middle/preterminal features and test which ones drive the upstream-conditioned terminal features.

---

## 6. Conclusion

First-divergence cross-patching turns a vague question -- "do late layers explain the base-to-instruct difference?" -- into a paired-checkpoint counterfactual. At the first token where released PT and IT checkpoints disagree, the IT-minus-PT late-stack replacement effect is partially portable but mostly upstream-conditioned: the IT late stack has a positive effect from PT upstream, yet its effect is much larger from IT-shaped upstream state. The result is positive across the five dense core families, stronger for general-purpose instruction-following descendants than a same-base domain fine-tune, coherent along two released stage lineages, and robust to targeted checks for readout choice, selected-token support, pre-late commitment, label orientation, and hybrid-state failure.

The mechanistic bridge is also graded rather than all-or-nothing. Middle windows are relatively more identity-selective, late and terminal windows are more margin/readout-sensitive, and terminal sparse features carry a concentrated part of the readout interaction. Those features are more causally important under IT-shaped upstream state, their native-IT activation pattern partially rescues the weak PT-upstream hybrid, and upstream/preterminal patches drive a measurable part of their mediated effect. Together with structured boundary-state closure, the picture is a portable-vs-conditioned late-readout decomposition in released dense base/instruct checkpoint contrasts, not a fully portable late-only update.
...
| Core-5 first-divergence interaction, matched/portable ratio, and portable share | §3.1 | B | `results/paper_synthesis/exp23_core5/`; `scripts/analysis/build_exp23_core5_synthesis.py` |
...
| G | Same-base recipe control, constrained continuation scoring, a blind short-completion judge audit, and two released dense lineages show that the result is not an automatic consequence of fine-tuning, a one-token artifact, or one final checkpoint accident. | Isolated causal attribution to SFT, DPO, or RLVR algorithms, broad completion-level behavioral interaction, or natural-rollout behavior for every recipe. |
...
All rows use raw-shared prompts, greedy top-1 first-divergence search with a real-token mask, and at most `128` generated tokens. Bootstrap unit is the prompt cluster within family.
...
The Core-5 synthesis combines stored per-family prompt-bootstrap estimates for the five families in §2.1. The matched/portable ratio reported in §3.1 is:
...
| Scope/readout | PT-up late effect | IT-up late effect | Interaction | Matched/portable ratio | Portable share | Native shift | Interaction share |
|---|---:|---:|---:|---:|---:|---:|---:|
...
**What this does not prove.** This is not full circuit recovery, and it does not prove feature monosemanticity. The boundary-state closure test is not recipe-unique and does not estimate completion-level behavior. OLMo is excluded from feature-level claims because its terminal crosscoder did not pass the reconstruction gate.

**Where to audit.** Crosscoder training, mediation, gating, feature rescue, structured boundary-state closure, handoff, autointerp, and structure-readout artifacts are listed in Appendix I.2.
...
**Claim supported.** The interaction is not merely an automatic consequence of fine-tuning or a one-token artifact, and fixed-support versions appear coherently along two released dense post-training lineages.

**Primary evidence.** On the same Llama-3.1-8B base, general-purpose instruction-following descendants show positive interaction on instruction/format supports while OpenMath2 does not. A constrained continuation bridge shows the instruction-following interaction persists beyond the selected first token. A blind short-completion judge audit detects key intervention contrasts while showing broad judged interaction is category-dependent. On Base->Final support, Tulu-3 and OLMo-2 both show partial SFT presence, and the DPO/preference checkpoint expresses most of the final measured interaction.
...
**Where to audit.** Recipe-structure, constrained-continuation, blind-judge, and stage-sweep artifacts are listed in Appendix I.2.

### G.1 Same-Base Recipe Structure

The recipe-structure check compares descendants of the same Llama-3.1-8B base. On instruction/format support, general-purpose instruction-following descendants are consistently upstream-conditioned, while the OpenMath2 domain-specialized descendant does not reproduce the same interaction.
...
Because this analysis re-scores the four cells in bf16 runtime conditions, `N=0` cell values do not reproduce the §3.1 estimates bit-exactly: median maximum drift is `0.125`, q99 is `0.375`, and `61/13664` comparisons exceed `0.5`. We therefore use aggregate horizon estimates rather than eventwise exact equality.
...
The data-quality checks pass: `30,047/30,047` judge requests produced valid structured outputs after retry, and same-text controls are judged tie/neutral `100%` of the time. The full base-to-descendant positive control is strong (`0.693` `[0.682, 0.704]`), so the judge can detect the released checkpoint contrast. The upstream-vs-late diagnostic is also positive (`0.654` `[0.643, 0.664]`), meaning descendant-shaped upstream state generally matters more for judged completions than a portable descendant-late-stack-only comparison.

The broad behavioral interaction itself is not a headline win. Overall it is `+0.0047` `[-0.0088, +0.0181]`; on the instruction-following slice it is `+0.0048` `[-0.0098, +0.0196]`. Category-level effects are mixed: safety is positive (`+0.076` `[+0.042, +0.110]`), GOV-FORMAT is small and marginal (`+0.0146` `[-0.0011, +0.0300]`), and GOV-CONV is negative (`-0.052` `[-0.079, -0.024]`). We therefore use this audit descriptively: it validates detectability and a safety-specific behavioral bridge, while the primary sequence-level claim remains the fixed-prefix and constrained-likelihood evidence.
...
| Small raw rerun | One 4B/7B family first-divergence cross-patching run and analysis. | 1-8 A100/H100 GPUs, depending on batching |
...
|---|---|---|
| Core-5 four-cell result and 32B scale check | Core synthesis scripts and first-divergence collectors | `core5_first_divergence`; `qwen25_32b_scale_check`; `results/paper_synthesis/exp23_core5` |
| Hybrid-state, token-selection, and pre-late controls | Validation and selection-control analyzers | `validation_controls`; `exp36_offmanifold_validation`; `exp37_random_prefix_baseline`; `exp40_prelate_commitment_control` |
| Late-localization motivation | Endpoint KL and late-MLP control synthesis | `late_localization_signatures`; `exp09`; `exp11`; `exp14`; `exp19` |
| Depth and terminal anatomy | Handoff, terminal-depth, and terminal-MLP analyzers | `depth_terminal_anatomy`; `exp20`; `exp21`; `exp31`; `exp32`; `exp33` |
| Sparse terminal feature bridge | Crosscoder mediation, gating, rescue, handoff, autointerp, and structure-readout analyses | `sparse_terminal_features`; `exp34`; `exp39`; `exp41`; `exp42`; `exp43`; `exp44` |
| Structured boundary-state closure | Static-chimera and structured-rescue analysis | `structured_boundary_state`; `exp48_static_chimera_sequence_validation` |
| Recipe, continuation, judge, and stage-lineage checks | Same-base recipe, constrained-continuation, blind-judge, Tulu, and OLMo analyzers | `recipe_stage_behavior`; `exp46`; `exp47`; `exp49`; `exp50`; `exp35` |
| Reviewer-facing CPU checks | Claim checker and minimal reproduction scripts | `scripts/reproduce/check_paper_claims.py`; `scripts/reproduce/reproduce_minimal.sh`; `scripts/reproduce/check_minimal_shard.py` |
