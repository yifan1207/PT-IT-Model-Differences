...
![Figure A40: Natural-rollout residual-opposition importance. Late residual-opposing MLP ablations hurt each model's own generated-token prediction much more in IT than PT for structured no-opposition and sign-flip interventions; same-magnitude random-removal controls show this is not a generic late-vector fragility effect.](../results/exp27_natural_rollout_residual_opposition_ntp/exp27_full_dense5_combined_20260430_2050/analysis/exp27_natural_rollout_ntp.png)

**Table A6: natural-rollout residual-opposition importance.** Dense-5 run over `600` prompts per PT/IT family. The analyzed table contains `372,144` generated-token source-position units after aggregating random seeds. `IT-PT` compares each model's own native rollout, so it should be read as deployment-style importance rather than a matched-history causal effect.
