...
Post-training changes a checkpoint's next-token preferences, but model diffing needs a way to locate how that change appears inside the forward pass. We introduce **first-divergence factorial diffing**: at the earliest shared-history prefix where a pretrained checkpoint (PT) and its post-trained descendant (IT) prefer different next tokens, we cross PT/IT upstream residual state with PT/IT downstream late stack and measure the IT-vs-PT divergent-token margin. The question is not whether transformer layers ever depend on upstream state; the question is whether the PT→IT change behaves like a portable late-only update or like a middle-to-late handoff. Across six dense PT/IT pairs, including Qwen2.5-32B, the upstream × late-stack interaction is positive in every family: the Dense-6 family-balanced estimate is `+2.44` logits (95% CI `[+2.35, +2.52]`), and the Gemma-removed Dense-5 estimate is `+1.71` `[+1.64, +1.78]`. The sign is label-aligned under a five-family permutation null (`p=5×10⁻⁵`) and dissociated from the simple late-only term: on a factual/reasoning stress test, the late-only term flips negative (`-1.18`) while the interaction remains positive (`+1.81`). An OLMo-2 stage-lineage case study shows the largest adjacent-stage jump at Base→SFT. Supporting tests give the anatomy of the handoff: middle-positioned MLP substitutions transfer token identity more often, late-positioned substitutions have larger margin/readout effects, and a residual-opposition ablation shows that this late geometry has measurable causal leverage without explaining the whole interaction. In the Dense-5 support set, removing the residual-opposing component of late IT MLP updates drops the interaction by `+0.258` logits (`[+0.225, +0.293]`, `9.8%` mediation), while sign-flipping it drops `+0.481` logits (`[+0.431, +0.536]`, `18.3%`); the effect is family-dependent and is strongest in Gemma. The contribution is a reusable model-diffing protocol plus a cross-family measurement of how post-training changes next-token formation at the first PT/IT disagreement.

---

## 1. Introduction

Suppose a pretrained checkpoint (PT) and its post-trained descendant (IT) read the same prefix and arrive at different next-token preferences — say PT prefers `.` and IT prefers `,`. This is a model-diffing question: what changed inside the forward pass when the checkpoint became instruction-following? The point is not that late transformer blocks ordinarily ignore upstream state; they do not. The point is whether the PT→IT change is portable as a late-only update, or whether post-training makes late computation effective only after earlier layers have shaped the residual stream toward the IT candidate.

Two empirical signatures made this counterfactual worth testing. First, IT checkpoints show delayed stabilization: decoded intermediate layers stay farther from the same checkpoint's final next-token distribution than PT layers do. Second, our geometry traces show a residual-opposing component in late IT MLP updates, with the largest IT-vs-PT shifts in some families (Appendix A.1). A direct ablation test shows that this component is not merely decorative: removing it reduces the Dense-5 upstream × late interaction by `+0.258` logits (`9.8%` of the full interaction), with a dose response under half-scaling and a larger drop under sign-flipping (§3.2). The effect is family-dependent and leaves most of the interaction intact, so we use it as support for the late-revision interpretation rather than as the main mechanism. The first-divergence factorial asks the sharper model-diffing question: when PT and IT first disagree, does the learned IT late computation carry the IT-token margin by itself, or only in compatibility with an IT-shaped upstream state?
...
**Result.** Across six dense PT/IT pairs, the upstream × late interaction is positive in every family. It is label-aligned under a PT/IT label-swap null on the five-family raw-record subset (`p=5×10⁻⁵`) and remains positive even when the simple late-only term reverses sign on factual/reasoning prompts. Magnitude varies by family, prompt regime, and generated position; we treat that variation as regime structure of the PT→IT change, not as a failure of the claim. An OLMo-2 stage-lineage case study shows the same interaction grows across released post-training stages, with the largest adjacent-stage jump at Base→SFT. Details for Gemma, position thresholds, Qwen2.5-32B, and OLMo stages are reported in §3.1 and Appendices A/F.

**Contributions.** The paper has three pieces.

1. **Method:** first-divergence factorial diffing, a reusable paired-checkpoint protocol for testing token-level PT/IT disagreements under shared history.
2. **Model-diffing finding:** post-training does not behave like a portable late-only update. The measured upstream × late interaction is positive across six dense PT/IT pairs, is label-aligned under a permutation null on the five-family raw-record subset, and remains positive even when the simple late-only term reverses on factual/reasoning prompts.
3. **Anatomy of the handoff:** middle-positioned MLP substitutions transfer first-divergent token identity more often than late-positioned substitutions, while late-positioned substitutions have larger margin/readout effects. Delayed stabilization and residual-opposing late MLP geometry provide the revision-side context; residual-opposition ablations show that this geometry has measurable, family-dependent leverage on the interaction; and the OLMo-2 stage lineage shows the compatibility relation growing across released post-training stages.
...
| **Dense-5 support set** | The five 4B-8B dense families used in supporting analyses that were not rerun or not pooled with Qwen2.5-32B: Gemma 3 4B, Llama 3.1 8B, Qwen 3 4B, Mistral 7B, and OLMo 2 7B. |
...
| **Early / middle / late windows** | Overlapping depth windows at comparable normalized depths across families (Appendix F gives boundaries). The middle/late identity-margin split in §3.3 is a relative-localization signal across overlapping windows, not a partition into disjoint subnetworks. |
...
| **Final-20% KL region** | Final normalized-depth segment used to summarize `KL(layer || own final)` in §3.2. |
...
Grouped rows labeled Dense-6 include Qwen2.5-32B. Rows labeled Dense-5 or Dense-4 deliberately exclude it because the corresponding supporting analysis was only run or only pooled for the five 4B-8B families. The Qwen2.5-32B PT/IT run repeats the core first-divergence factorial, identity/margin, MLP write-out, and raw-KL bridge analyses; its standalone details are reported in Appendix A.7, but its Exp23 interaction is included in the Dense-6 §3.1 estimate. A non-pooled OLMo-2 7B stage-progression case study applies the same first-divergence factorial to released Base, SFT, DPO, and Instruct/RLVR checkpoints; it is reported in §3.1 and Appendix A.8 as local transition evidence, not as an additive decomposition of the full Base→Instruct/RLVR contrast.
...
| Behavioral sanity check | **Natural decoding:** PT/intervened PT and IT/intervened IT use their intended generation setup. | Checks output movement, not circuit identification. |
...
Secondary diagnostics are demoted to appendices. Commitment summaries and raw-lens variants address metric/probe robustness; `δ`-cosine is a geometric marker of late revision rather than a complete mechanism by itself; residual-opposition ablations test how much that geometric component mediates the first-divergence interaction; Gemma feature-level analyses are supplementary.
...
**Dissociation from the simple late-only effect.** On a factual/reasoning stress test, the IT late stack from PT upstream moves *against* the IT token (`-1.18` logits, 95% CI `[-1.26, -1.09]`), while the upstream × late interaction remains positive (`+1.81` logits, 95% CI `[+1.72, +1.90]`). The simple late-only term therefore tracks prompt regime, but the interaction does not collapse with it. Appendix F gives the factual/reasoning, per-family, and subgroup breakdowns.

**Label alignment.** A label-swap control asks whether the sign is PT/IT-oriented rather than an arbitrary orientation. On the five-family raw-record subset, preserving each prompt's four factorial cell values but randomly swapping PT/IT labels gives a null centered near zero (99.9th percentile `+0.239` logits), while the observed Dense-5 interaction is far outside it (`+2.64`, `p=5.0e-5` with `20,000` permutations). This does not replace the Dense-6 interval evidence above; it rules out a metric-construction artifact in the raw-record subset where the null can be recomputed directly.
...
| Generated position `≥5`, Gemma removed | `+0.80` `[+0.70, +0.90]` | Llama family interval compatible with zero in the five-family subset. |
| Factual/reasoning stress test (Dense-5) | `+1.81` `[+1.72, +1.90]` | Persists outside governance/register-heavy holdout. |
| OLMo-2 staged lineage | Base→SFT `+0.78`; SFT→DPO `+0.14`; DPO→RLVR `+0.016`; Base→RLVR `+1.93` | Non-pooled local-transition case study over released OLMo stages. |
| Label-swap null (Dense-5 raw records) | observed `+2.64`; null 99.9% `+0.239`; `p=5×10⁻⁵` | Interaction is PT/IT-label aligned where the raw null is recomputed. |

All rows above use the common-IT 2×2 interaction readout. The factual/reasoning run also reports the PT-upstream late-only term (`-1.18` logits, 95% CI `[-1.26, -1.09]`) as a stress-test qualifier — this is the simple late-only effect that flips sign while the interaction stays positive.

The OLMo-2 staged-lineage case study checks a different scope question: how the same estimand changes across released post-training stages. The interaction is already substantial at Base→SFT, remains positive but smaller for SFT→DPO and DPO→RLVR, and is largest for the full Base→RLVR contrast. This supports the interpretation that post-training creates or amplifies a middle-to-late compatibility relation rather than merely exposing a generic transformer dependency. Appendices A.7-A.8 give the Qwen2.5-32B and OLMo details.

**Companion controls (identity/margin decomposition and late-window context).**

| Check | Result | Interpretation |
|---|---:|---|
| Raw-shared PT-host IT-token transfer (Dense-5 support set) | middle `26.0%` `[24.5%, 27.7%]`; late `17.6%` `[16.2%, 18.9%]` | Identity split survives without native IT templates; Qwen2.5-32B repeats the same ordering in Appendix A.7. |
| Native IT-template IT-host margin drop (Dense-5 support set) | late `13.25` logits `[12.91, 13.61]`; middle `12.01` `[11.66, 12.35]` | Native prompting makes late margin/readout strongest. |
| Matched random residual projection (Dense-5 support set, final-20% KL) | true late graft `+0.327` nats `[+0.298, +0.359]`; random `+0.003` `[-0.002, +0.008]` | Late MLP graft is not a generic late-window perturbation. |

### 3.2 Layerwise context: delayed stabilization, residual opposition, and late-window localization

> *TL;DR. Context for why we tested late windows: IT models stay farther from their own final-layer prediction than PT models do, and late IT MLP updates have a residual-opposing geometry consistent with revision. Late-MLP grafts have the largest tested leverage on the delay, with specificity confirmed against a matched random control (`+0.327` nats vs `+0.003`). A targeted Exp26 ablation shows that residual opposition has measurable leverage on the first-divergence interaction: removing it drops the Dense-5 interaction by `+0.258` logits (`9.8%`), and sign-flipping it drops `+0.481` logits (`18.3%`). Because the effect is family-dependent and most of the interaction remains, we treat this as support for the late-revision account rather than as a complete mechanism.*

The §3.1 factorial result is computed on raw logits at the divergent prefix and does not depend on a learned probe; this section provides the layerwise and geometric context for why we tested late windows in the first place. Under native free-running decoding, IT models remain farther from their own final next-token distribution than PT models do through much of the forward pass. Under the tuned lens, the dense-family IT-minus-PT `KL(layer || own final)` gap is positive in the early, middle, and late thirds of the network (`+0.62`, `+0.54`, and `+0.33` nats), and raw-lens variants preserve the qualitative ordering.
...
Exp26 provides a targeted dynamic probe of that geometry. At each first-divergence prefix, we rerun the residual-state × late-stack factorial while modifying only the residual-opposing component of late IT MLP updates inside the IT late stack. In the Dense-5 support set, removing that opposing component (`noopp`) drops the full upstream × late interaction by `+0.258` logits (95% CI `[+0.225, +0.293]`), or `9.8%` of the full interaction. Halving the component gives an approximately half-sized drop (`+0.121` logits, `[+0.100, +0.144]`; `4.6%` mediation), while sign-flipping it gives a larger drop (`+0.481` logits, `[+0.431, +0.536]`; `18.3%` mediation). Norm-preserving removal is nearly identical to ordinary removal (`+0.253` logits, `[+0.221, +0.288]`), so the effect is not just update-length shrinkage. A PT-level-opposition control gives a similar drop (`+0.243` logits, `[+0.216, +0.270]`), suggesting that the component participates in late-stack computation rather than acting as a standalone margin vector.

A symmetric PT-late-target check is a useful guardrail. On the same first-divergence units, removing the analogous residual-opposing component inside the PT late stack also reduces the interaction (`+0.186` logits, `[+0.169, +0.203]`), and sign-flipping it reduces the interaction by `+0.261` logits (`[+0.236, +0.286]`). Under the Dense-5 mean these PT-target drops are smaller than the corresponding IT-target drops, but the family-median and Gemma-removed summaries show that PT late opposition is also a real finalization mechanism (Appendix A.9). The safe conclusion is therefore not that post-training invents residual opposition from scratch or that this component explains the interaction across families. It is that residual opposition is one geometry-defined part of late finalization, and in the IT late stack it has measurable, family-dependent causal leverage on the PT-to-IT first-divergence interaction.
...
> *TL;DR. Within the Dense-5 support-set decomposition, middle-positioned MLP substitutions transfer token identity more often than late-positioned (`26%` vs `18%`; both well below 50%, so neither window alone controls token identity — this is a relative-localization signal). Late-positioned substitutions show larger margin/readout effects. Qwen2.5-32B repeats the same middle-over-late identity ordering in Appendix A.7, but the full table below keeps the common Dense-5 bootstrap scope.*
...
![Figure 5: Candidate transfer and local late support. First-divergence tests show that middle substitutions transfer PT-vs-IT token identity more, while late IT MLP windows provide the strongest native IT-token support. The full late MLP update carries the IT-vs-PT margin write-in in this proxy. The residual-opposing component is not the local margin vector, but Exp26 shows it has measurable, family-dependent leverage on the dynamic upstream × late interaction.](../results/paper_synthesis/exp20_exp21_handoff_synthesis.png)

Putting both sides together: middle windows are more diagnostic of which PT- or IT-like token candidate is exposed, while late windows interact with upstream state to shape the final margin. Mid and late windows must be modeled jointly under this readout — neither in isolation captures the PT/IT disagreement. The delayed-stabilization pattern in §3.2 is the layerwise trace of this handoff: upstream state already encodes much of the PT/IT decision, and late computation changes the margin only in compatibility with that state. Exp26 adds a guarded geometry check: removing the residual-opposing component measurably weakens the interaction while leaving most of it intact. Appendix A gives the token-flow chronology and residual-opposing component decomposition.
...
**Geometry interpretation.** Late IT-MLP updates have a substantial component opposing the residual stream entering them, a geometry consistent with revision or reconciliation rather than simple additive accumulation. The residual-opposing component is not itself the local margin write-in under the Exp21 pure-IT proxy: it contributes `-0.0046` logits to the IT-vs-PT margin (95% CI `[-0.009, -0.001]`), while the full late-MLP update contributes `+0.768` logits (95% CI `[+0.729, +0.805]`). Exp26 shows that this component is not purely epiphenomenal in the full dynamic late stack: removing it reduces the Dense-5 upstream × late interaction by `+0.258` logits (`[+0.225, +0.293]`; `9.8%` mediation), and sign-flipping it removes `+0.481` logits (`[+0.431, +0.536]`; `18.3%`). The correct inference is narrow. Residual opposition is a measurable, family-dependent support factor for late reconciliation, not the local margin vector and not a complete explanation of the upstream × late interaction. This keeps the geometry useful without making it the paper's main causal claim.
...
The appendices are organized by purpose rather than by experiment ID. Appendix figures use local monotone labels (`Figure A1`-`A40` in Appendix A, `Figure B1` in Appendix B) rather than legacy run numbers; appendix tables use `Table A1`-`A6`. A reviewer checking a specific claim can use this map:
...
| Exp21 raw-shared residual-opposing component | `-0.000019` | `[-0.000062, +0.000026]` | Residual-opposition geometry is not the margin vector. |
...
### A.8 OLMo-2 stage-progression case study

Exp25 uses the released OLMo-2 7B Base, SFT, DPO, and Instruct/RLVR checkpoints to ask whether the upstream × late interaction appears in adjacent post-training transitions, not only in the full Base→Instruct/RLVR contrast. This is a single-lineage case study and is not pooled with the Dense-6 core estimate. The key caution is that each adjacent transition has its own first-divergence events and divergent token pair, so the adjacent estimates are local transition measurements rather than additive components of the full Base→RLVR number.

**Table A5: OLMo-2 7B local transition estimates.** All rows use raw-shared first-divergence factorial diffing under the common later-stage readout. The "later-stage late effect" and "earlier-stage late effect" columns are the simple late-stack effects from later-shaped and earlier-shaped upstream states, respectively.

| Transition | Interaction (95% CI) | Later-stage late effect | Earlier-stage late effect | IT-token transfer proxy | Interpretation |
|---|---:|---:|---:|---:|---|
| Base→SFT | `+0.782` `[+0.681, +0.882]` | `+1.328` | `+0.546` | mid+late `40.8%` vs mid `24.1%` | Strongest adjacent-stage interaction. |
| SFT→DPO | `+0.135` `[+0.114, +0.157]` | `+0.821` | `+0.686` | mid+late `57.1%` vs mid `30.7%` | Smaller preference-stage refinement. |
| DPO→RLVR | `+0.016` `[+0.008, +0.025]` | `+0.098` | `+0.082` | mid+late `43.8%` vs mid `35.0%` | Positive but tiny local addition. |
| Base→RLVR | `+1.930` `[+1.749, +2.112]` | `+2.698` | `+0.768` | mid+late `36.0%` vs mid `18.4%` | Full contrast remains large. |

The stage-progression result fits the paper's main reading: post-training creates or amplifies a compatibility relation between upstream state and late computation, with the largest local OLMo jump appearing at SFT and a much smaller additional RLVR-stage change under this readout. Exp20 collection completed all four transitions at `600/600` records; valid first-divergence units were `551` for Base→SFT, `557` for SFT→DPO, `466` for DPO→RLVR, and `586` for Base→RLVR. Preflight checks found no raw-prompt token-ID mismatches across the stage pairs; all variants use `GPT2TokenizerFast`, shared vocab size `100278`, EOS `<|endoftext|>` (`100257`), and pad `<|pad|>` (`100277`). The SFT, DPO, and Instruct/RLVR tokenizers share the same chat-template hash, while the raw-shared factorial uses raw text for both branches.

![Figure A39: OLMo-2 7B stage progression. Local first-divergence transition estimates show positive upstream x late interaction at each released stage transition, with the strongest adjacent transition at Base→SFT and a large full Base→RLVR contrast.](../results/paper_synthesis/exp25_olmo_stage_full_20260428_0905/olmo_stage_progression.png)

### A.9 Residual-opposition mediation inside the late stack

Exp26 tests whether the residual-opposing geometry is only descriptive or actually contributes to the first-divergence upstream × late interaction. It reruns the Dense-5 raw-shared residual-state × late-stack factorial at the same first-divergence prefixes as Exp23, but modifies only the component of late IT MLP updates that points opposite the incoming residual stream. The target is therefore narrow: mediation by one geometry-defined component inside the late IT stack, not a complete feature-level circuit decomposition.

![Figure A40: Residual-opposition mediation controls. Removing, half-scaling, sign-flipping, PT-leveling, or random-orthogonalizing the residual-opposing component of late IT MLP updates inside the first-divergence factorial.](../results/exp26_residual_opposition_mediation/exp26_dense5_full_a100x8_20260429_111420/analysis/exp26_mediation_plot.png)

**Table A6: residual-opposition mediation controls.** The full Dense-5 upstream × late interaction under the common-IT readout is `+2.635` logits over `2,983` prompt clusters. Positive drops mean the modified late IT stack loses part of that interaction relative to the unmodified late IT stack.

| Variant | Interaction drop vs full IT late | 95% CI | Mediation fraction | Interpretation |
|---|---:|---:|---:|---|
| Half-scale opposition | `+0.121` | `[+0.100, +0.144]` | `4.6%` | Dose response: half the component removes about half the no-opposition effect. |
| Remove opposition (`noopp`) | `+0.258` | `[+0.225, +0.293]` | `9.8%` | Core ablation: this geometry-defined component has measurable leverage. |
| Norm-preserving no opposition | `+0.253` | `[+0.221, +0.288]` | `9.6%` | The effect is not explained by merely shrinking MLP-update norm. |
| PT-level opposition | `+0.243` | `[+0.216, +0.270]` | `9.2%` | Replacing the IT opposing component with a PT-level counterpart does not restore the full interaction. |
| Random orthogonal replacement | `+0.322` | `[+0.284, +0.361]` | `12.2%` | A norm-matched random replacement does not restore the structured effect. |
| Flip opposition | `+0.481` | `[+0.431, +0.536]` | `18.3%` | The sign of the opposing component matters; adversarial reversal is more damaging. |
...
**Table A7: paired IT-late versus PT-late residual-opposition targets.** Positive `IT-PT` means the IT-late-target intervention damages the upstream × late interaction more than the PT-late-target intervention on the same first-divergence units.

| Scope | Variant | IT target drop | PT target drop | IT-PT drop | 95% CI for IT-PT |
|---|---:|---:|---:|---:|---:|
| Dense-5 mean | Remove opposition | `+0.258` | `+0.186` | `+0.072` | `[+0.028, +0.116]` |
| Dense-5 mean | Norm-preserving removal | `+0.253` | `+0.165` | `+0.088` | `[+0.045, +0.130]` |
| Dense-5 mean | Flip opposition | `+0.481` | `+0.261` | `+0.220` | `[+0.154, +0.286]` |
| Dense-5 mean | Random orthogonal replacement | `+0.322` | `+0.132` | `+0.190` | `[+0.137, +0.242]` |
| Family median | Remove opposition | `+0.040` | `+0.104` | `-0.269` | `[-0.345, -0.164]` |
| Gemma removed | Remove opposition | `-0.119` | `+0.264` | `-0.383` | `[-0.423, -0.344]` |
...
|---|---:|---:|---:|
| all positions | `4,380` | `+2.44` `[+2.35, +2.52]` | `+1.71` `[+1.64, +1.78]` |
| positions `>=1` | `2,339` | `+2.08` `[+1.96, +2.19]` | `+1.16` `[+1.08, +1.24]` |
| positions `>=3` | `1,428` | `+1.43` `[+1.30, +1.57]` | `+0.83` `[+0.75, +0.91]` |
| position `>=5` | `922` | `+1.48` `[+1.28, +1.68]` | `+0.80` `[+0.70, +0.90]` |
| position `>=10` | `352` | `+1.39` `[+0.82, +1.97]` | `+1.01` `[+0.81, +1.21]` |
...
| Residual-opposition mediation inside the late IT stack | `uv run python scripts/analysis/analyze_exp26_residual_opposition_mediation.py --exp26-root results/exp26_residual_opposition_mediation/exp26_dense5_full_a100x8_20260429_111420 --exp23-root results/exp23_midlate_interaction_suite/exp23_dense5_full_h100x8_20260426_sh4_rw4 --out-dir results/exp26_residual_opposition_mediation/exp26_dense5_full_a100x8_20260429_111420/analysis --models gemma3_4b llama31_8b qwen3_4b mistral_7b olmo2_7b --prompt-mode raw_shared --event-kind first_diff --n-boot 2000` | `results/exp26_residual_opposition_mediation/exp26_dense5_full_a100x8_20260429_111420/analysis/exp26_summary.json`; `exp26_effects.csv`; `exp26_red_team_report.md` | full interaction `+2.635`; remove-opposition drop `+0.258` `[+0.225, +0.293]` (`9.8%`); half-scale `+0.121` `[+0.100, +0.144]`; flip `+0.481` `[+0.431, +0.536]` (`18.3%`); PT-level `+0.243` `[+0.216, +0.270]` |
| Paired PT-late residual-opposition target | same analyzer with `--late-target pt`; paired comparison generated from IT-target and PT-target runs | `results/exp26_residual_opposition_mediation/exp26_pttarget_dense5_ptlate_combined_20260429_2125/analysis/`; `results/exp26_residual_opposition_mediation/pt_vs_it_target_comparison_20260429_2135/` | PT-target remove-opposition drop `+0.186` `[+0.169, +0.203]`; flip `+0.261` `[+0.236, +0.286]`; paired Dense-5 IT-PT noopp `+0.072` `[+0.028, +0.116]`; family-median noopp favors PT target, so this is evidence for generic late finalization plus family-dependent post-training reshaping, not IT-only creation |
...
| Residual-opposition mediation controls | `src/poc/exp26_residual_opposition_mediation/`; `scripts/run/run_exp26_residual_opposition_mediation_runpod.sh`; `scripts/analysis/analyze_exp26_residual_opposition_mediation.py` | `results/exp26_residual_opposition_mediation/exp26_dense5_full_a100x8_20260429_111420/analysis/`, `results/exp26_residual_opposition_mediation/exp26_pttarget_dense5_ptlate_combined_20260429_2125/analysis/`, `results/exp26_residual_opposition_mediation/pt_vs_it_target_comparison_20260429_2135/` |
