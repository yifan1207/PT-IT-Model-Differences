...
Layer-stage accounts of late transformer computation — refinement, sharpening, calibration — presuppose that late layers read upstream-shaped residual state. They do not test the counterfactual: at the prefix where paired pretrained (PT) and post-trained (IT) checkpoints first prefer different next tokens, does the same IT late stack have the same effect when driven by a PT-shaped upstream state as by an IT-shaped one? We introduce first-divergence factorial diffing to estimate that interaction. Across five released dense PT/IT checkpoint pairs (Gemma 3 4B, Llama 3.1 8B, Qwen 3 4B, Mistral 7B, OLMo 2 7B), the upstream-state x late-stack interaction on the IT-vs-PT divergent-token margin is **positive in every family, with each per-family interval excluding zero**, and **label-aligned**: a permutation that randomly reorients PT/IT labels gives a null whose 99.9th percentile is `+0.239` logits, while the observed family-balanced interaction sits far outside that null (`p=5.0e-5`). The interaction is also **dissociated from the simple late-only term**: on a factual/reasoning stress test the IT-late-from-PT-upstream effect flips negative (`-1.18` logits) while the interaction stays positive (`+1.81`). The family-balanced point estimate is `+2.64` logits (95% CI `[+2.54, +2.74]`); excluding the largest-magnitude family (Gemma) leaves `+1.77` `[+1.69, +1.86]`, and after dropping all first-generated-token events the dense-5 estimate is `+2.25` `[+2.11, +2.38]` (Gemma-removed `+1.14`). Supporting first-divergence tests separate candidate identity from margin: middle-positioned substitutions transfer token identity more often (`26.0%` vs `17.6%`), while late-positioned substitutions dominate margin/readout. The contribution is a reusable measurement battery for natural first disagreements in paired dense PT/IT model diffing, not circuit recovery or a general mechanism for instruction following.
...
**Main claim.** At natural first PT/IT next-token disagreements, late-stack effects on the IT-vs-PT margin are strongly state-conditioned. This is an intervention-scoped claim about specified hybrid forward passes and readouts, not a claim of circuit recovery or of a universal instruction-following module.

Across five released dense PT/IT checkpoint pairs, the upstream-state x late-stack interaction is positive in every family and label-aligned. A label-swap permutation that preserves each prompt's four cell values but randomly reorients PT/IT puts the observed family-balanced interaction far outside the null 99.9th percentile (`+0.239` logits; `p=5.0e-5`); on a factual/reasoning stress test the simple late-only PT-upstream term flips sign (`-1.18` logits) while the interaction stays positive (`+1.81`). The result is therefore both label-aligned and dissociated from the simple late-only effect.

The family-balanced point estimate is `+2.64` logits (95% CI `[+2.54, +2.74]`); each per-family interval excludes zero (median family `+1.85`, trimmed family mean `+1.95`), and the Gemma-removed mean is `+1.77` `[+1.69, +1.86]`. Dropping all first-generated-token events leaves the dense-5 estimate at `+2.25` `[+2.11, +2.38]` (Gemma-removed `+1.14`); at generated position `>=3` it is `+1.52` `[+1.36, +1.68]` with all five family-specific intervals above zero. This is a selected-event claim about natural first PT/IT disagreements, not a random-token statement; Section 3.1 treats Gemma as the largest-magnitude family and early response formation as the strongest regime, not as hidden assumptions.
...
A companion first-divergence decomposition separates candidate identity from final margin. Middle-positioned window substitutions transfer token identity more often, while late-positioned window substitutions more strongly affect margin/readout.

The novelty is methodological. Layer-localization work can say where task information or output sharpening tends to appear; it does not test the counterfactual "run the same IT late stack from the PT upstream state at the exact token where PT and IT first disagree." The 2x2 factorial makes that counterfactual explicit. A state-independent late effect is an operational null for this measurement, not a claim that prior authors believed late layers ignore their inputs. The point is to measure how far the paired PT/IT contrast departs from that null, whether the sign is label-aligned, and how the magnitude changes under family, position, and prompt-regime stratification.
...
All causal language in this paper is intervention-scoped. It means that replacing an upstream state, late stack, or MLP window in a constructed forward pass changes a specified readout. It does not mean that the hybrid state is a natural state, that the readout is the only meaningful behavior metric, or that the intervention has recovered a feature-level circuit. Matched histories, the existing MLP-graft random control, label-swap controls, and raw-shared validation reduce obvious artifacts, but they do not remove the need to interpret each intervention with its metric and direction fixed.
...
Layerwise context comes from two supporting runs: a native free-running 2,936-prompt convergence analysis, and a dense-family 600-prompt endpoint-matched run with raw and tuned probes. The behavioral sanity check uses a frozen 600-prompt subset emphasizing conversational, register-sensitive, safety, and format-sensitive items; it checks aggregate output movement with LLM judging and a completed two-rater human audit, but it is not used to identify the internal circuit.
...
Under the common-IT readout, swapping in the IT late stack shifts the IT-vs-PT margin by `+0.572` logits (95% CI `[+0.494, +0.647]`) from a PT upstream state, but by `+3.207` logits (95% CI `[+3.095, +3.321]`) from an IT upstream state. Conditional on these five released families, the matched 2x2 decomposition gives a late-stack main effect of `+1.890` logits (95% CI `[+1.805, +1.975]`), a larger upstream-context effect of `+4.239` logits (95% CI `[+4.105, +4.376]`), and a positive interaction of `+2.635` logits (95% CI `[+2.538, +2.736]`). The common-PT readout agrees (`+0.609`, 95% CI `[+0.539, +0.677]`, versus `+3.218`, 95% CI `[+3.109, +3.333]`, for IT-late given PT versus IT upstream).

This is the core non-additivity estimate. Under this readout, a state-independent late effect would produce similar IT-vs-PT margin shifts from PT-shaped and IT-shaped upstream states. Instead, the PT-upstream effect is much smaller. The result is an intervention effect on the specified margin; it does not by itself identify a circuit.

**Label alignment.** A label-swap control tests whether the interaction is aligned with the PT/IT labels rather than an arbitrary sign convention. Re-scoring each cell by the token favored by its late stack, the IT late stack gains `+5.56` logits from IT-shaped rather than PT-shaped upstream state, while the PT late stack gains `+2.92` logits from PT-shaped rather than IT-shaped upstream state; their difference is the same `+2.64` interaction. A permutation that preserves each prompt's four factorial cell values but randomly swaps the PT/IT label orientation gives a null centered near zero (99.9th percentile `+0.239` logits), with the observed amplification far outside the null (`p=5.0e-5`, the minimum resolvable with `20,000` permutations). The interaction is therefore label-aligned, not a metric-construction artifact.

**Dissociation from the simple late-only effect.** On a factual/reasoning stress test using distinct prompt IDs from a content/reasoning slice, the IT late stack from PT upstream moves *against* the IT token (`-1.18` logits, 95% CI `[-1.26, -1.09]`), while the upstream-late interaction stays positive (`+1.81` logits, 95% CI `[+1.72, +1.90]`). The simple late-only effect is therefore not domain-portable; the interaction is. The interaction is positive for both factual prompts (`+1.96`) and reasoning prompts (`+1.35`). The primary holdout is dominated by governance, register, safety, and formatting prompts, where post-training-specific mid-to-late cooperation is expected to be strongest and PT upstream state can still be partly compatible with the IT late stack. Factual/reasoning prompts are less dominated by assistant-register and safety-format behavior, so the simple late-only term need not keep the same sign.

**Position-stratified regimes (headline).** The interaction is largest at first response formation (`+3.01` logits at generated position 0) and remains positive through generated position `>=3` in every family (`+1.52`, all five family-specific intervals above zero); after dropping position 0 entirely the dense-5 estimate is `+2.25` (Gemma-removed `+1.14`). Position selects different disagreement regimes — format/safety/register openings, mid-response content disagreements, later shared-history disagreements — and the magnitude differences across them are part of the result. Detailed per-position thresholds appear below.

![Figure 1: Core estimate: late-stack effects are non-additive with upstream state under this readout. First-divergence factorial diffing crosses upstream residual state with downstream late stack while holding the scoring readout fixed. The right panel shows family-level interaction CIs: the upstream-state x late-stack interaction is positive in every dense family, with each per-family 95% interval excluding zero, and remains positive without Gemma (`+1.77` logits, `[+1.69, +1.86]`). A label-swap control preserving each prompt's four cell values but randomly reorienting PT/IT places the observed family-balanced interaction far outside the null 99.9th percentile (`+0.239` logits; `p=5.0e-5`), so the effect is label-aligned rather than an artifact of metric construction. The simple PT-upstream and IT-upstream late-stack effects (`+0.57` and `+3.21` logits) are the components that enter the interaction; we do not use their ratio as the inferential quantity because the small PT-upstream denominator inflates ratios. Magnitude varies by family and by first-divergence position (Section 3.1, Appendix F).](../results/exp23_midlate_interaction_suite/exp23_dense5_full_h100x8_20260426_sh4_rw4/analysis/exp23_midlate_interaction.png)
...
| Dense-5 (family-balanced) | `2,983` | `+0.57` | `+3.21` | `+2.64` `[+2.54, +2.74]` |
| Dense-4 (Gemma removed) | `2,383` | — | — | `+1.77` `[+1.69, +1.86]` |
...
**Gemma as the largest-magnitude family.** Gemma is not used as the representative magnitude. It has the largest descriptive ratio (`60.5x`, mostly because the PT-upstream denominator is only `+0.10` logits), the smallest tuned-vs-raw lens improvement in the convergence-gap layerwise plots, and the most late-concentrated MLP weight-change profile (Tables A1-A3). These are diagnostics for why we report Gemma separately, not evidence that those diagnostics cause Gemma's larger interaction. The first-divergence factorial itself is a raw-logit margin measurement, so Gemma's large interaction is not a tuned-lens artifact; showing that the late-concentrated weight-change profile explains the larger Gemma effect would require a within-family mediation test or more families, not a correlation across five checkpoints. Ratios are kept descriptive because small PT-upstream denominators inflate them; the logit interaction is the inferential quantity.

**Position thresholds (detailed).** Position 0 is not a chat-template or BOS artifact: the primary factorial uses raw-shared prompts for both PT and IT branches, validates identical prompt token IDs, and evaluates the first generated token after the full prompt-conditioned residual state. Recomputing the family-balanced estimator after dropping all position-0 records gives `+2.25` logits (95% CI `[+2.11, +2.38]`; Gemma removed `+1.14` `[+1.05, +1.23]`). Restricting to first divergences at generated position `>=3` gives `+1.52` logits (95% CI `[+1.36, +1.68]`; Gemma removed `+0.79` `[+0.70, +0.88]`), with all five family-specific intervals above zero. At generated position `>=5`, the pooled estimate remains positive (`+1.64`, 95% CI `[+1.39, +1.88]`; Gemma removed `+0.83` `[+0.71, +0.94]`), but the cell is lower-powered (`495` records) and Llama's family-specific interval is compatible with zero. Appendix F gives the category-mix audit for the `>=3` subset.

**Subgroup characterization.** Within the primary holdout, subgroup checks preserve the same sign without changing the estimand. The interaction is positive for conversational/governance prompts (`+2.05` logits), format prompts (`+3.61`), and safety prompts (`+2.83`), and it is also positive when stratified by first-divergent IT-token type: content-like (`+2.50`), format (`+2.60`), and function/other (`+2.81`). These strata characterize where the primary estimate comes from; they are not additional headline claims.

**Regime and control summary.**

The primary regime and control checks stay on this estimand:
...
| Generated position `>=3`, Gemma removed | Common-IT 2x2 interaction | `+0.79` logits `[+0.70, +0.88]` | Later-position dense-family regime remains positive without the largest-magnitude family. |
| Generated position `>=5` | Common-IT 2x2 interaction | `+1.64` logits `[+1.39, +1.88]` | Pooled later-position regime; per-family power is lower. |
| Generated position `>=5`, Gemma removed | Common-IT 2x2 interaction | `+0.83` logits `[+0.71, +0.94]` | Lower-powered later-position regime; Llama family interval is compatible with zero. |
...
The behavioral evidence is a consistency check, not a standalone benchmark. LLM judging and the completed human audit agree directionally on the primary pairwise contrasts, but high tie/both-bad rates (`44.3-77.0%`) and heterogeneous kappa keep the behavioral result from carrying the internal intervention claim. The internal result does not depend on those labels.
...
First-divergence position is an informative axis of the phenomenon, so we stratify the primary Exp23 interaction by generated position. The estimate is largest at the first generated token and changes in later shared-history regimes, while remaining positive in the later-position bins:
