...
**First-divergence position distribution.** First-divergence prefixes are selected events, not random token positions. In the primary first-divergence holdout, the `2,983` dense-family valid events occur early: mean generated position `2.17`, median `0`, with `1,499` at the first generated token and `2,488` within positions `0-4`. The IT-side divergent token categories are `1,265` content, `685` format, and `1,033` function/other tokens. The primary factorial stores local intervention cells rather than arbitrary-position full logits; as a trajectory proxy, the raw-shared auxiliary analysis gives branch-token cluster entropy `0.92` nats at first divergence versus `1.62` nats for a uniformly sampled generated position. Because the early-position skew can affect magnitude, Section 3.1 reports position-stratified versions of the primary interaction, including first divergences at generated positions `>=3` and `>=5`.
...
### 3.3 Supporting decomposition: middle-positioned windows transfer identity; late-positioned windows affect margins
...
| Mid+late KL factorial, residual-state x late-stack interaction, subgroup/position characterization, label-swap control, and content/reasoning extension | `src/poc/exp23_midlate_interaction_suite/`; `src/poc/exp23_midlate_kl_factorial/`; `scripts/run/run_exp23_midlate_interaction_suite.sh`; `scripts/run/run_exp23_exp21_content_reasoning_extension.sh`; `scripts/analysis/analyze_exp23_midlate_interaction_suite.py`; `scripts/analysis/analyze_exp23_interaction_subgroups.py`; `scripts/analysis/analyze_first_divergence_position_sensitivity.py`; `scripts/analysis/analyze_exp23_compatibility_permutation.py` | `results/exp23_midlate_interaction_suite/exp23_dense5_full_h100x8_20260426_sh4_rw4/analysis/`, `results/exp23_midlate_interaction_suite/exp23_content_reasoning_residual_20260427_0930_h100x8/analysis/`, `results/paper_synthesis/exp23_position_sensitivity_table.csv`, `results/paper_synthesis/exp23_position_sensitivity_per_family.csv` |
