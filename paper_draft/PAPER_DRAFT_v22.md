...
|---|---|---|
| First-divergence factorial diffing | Patch upstream residual state at the late boundary, then run PT or IT late stack/readout at a first-divergence prefix. | Estimates the primary upstream-late interaction. |
...
**Behavioral sanity check.** The free-running behavioral follow-up uses a frozen 600-prompt subset of `eval_dataset_v2.jsonl`, emphasizing conversational prompts, assistant-register prompts, benign and harmful safety prompts, and format-sensitive items. The main behavioral estimates are LLM-judge results, with a completed two-rater blind human audit of the primary pairwise contrasts and pointwise judge-calibration labels. This evidence checks aggregate behavioral consistency; it is not used to identify the internal mechanism.
...
The simple late effect from PT upstream is `Y(U_PT,L_IT) - Y(U_PT,L_PT)`. The simple late effect from IT upstream is `Y(U_IT,L_IT) - Y(U_IT,L_PT)`. The upstream-late interaction is:
...
### 3.4 Secondary behavioral sanity check
...
This evidence is deliberately secondary. A completed two-rater blind human audit agrees directionally on the four primary pairwise contrasts, but unresolved labels remain common (`44.3-77.0%`) and rater agreement is heterogeneous: pairwise Cohen's kappa ranges from `0.17` to `0.56` across the four-label contrasts and from `0.17` to `0.54` after collapsing unresolved labels. These limits keep the behavioral result from carrying the mechanistic claim; Appendix B gives the judge design, human-audit tables, unresolved rates, and kappa caveats.
...
### A4: Matched-prefix localization and diagnostic behavioral figures
...
**Metric surface area.** We report several convergence summaries because the layerwise pattern is trajectory-shaped, but the internal claims are intentionally pinned to a small ladder: upstream-late interaction as the primary estimand, first-divergent-token identity/margin as the local decomposition, and native KL-to-own-final, endpoint-free same-history JS, and symmetric final-20% graft/swap effects as layerwise context and localization support. Per-family or auxiliary-metric deviations are treated as diagnostics unless they agree with that ladder. Judged behavior is a separate consistency bridge, not part of the evidential core for the internal mechanism.
...
**Label-swap interaction control.** The Exp23 interaction can be reparameterized as a comparison between two matched-stage boosts. Let `Y = logit(IT token) - logit(PT token)`. The IT-late boost is `Y(U_IT,L_IT) - Y(U_PT,L_IT)`, while the PT-late boost is `Y(U_IT,L_PT) - Y(U_PT,L_PT)` after flipping the margin into the PT-token direction. Their difference equals the upstream-late interaction. On the primary holdout, the IT-late boost is `+5.56` logits and the PT-late boost is `+2.92`, giving `+2.64`; on the content/reasoning extension, the corresponding values are `+4.37` and `+2.69`, giving `+1.68`. A label-swap permutation that preserves all four cell values per prompt/event but randomly swaps PT/IT orientation yields null distributions centered near zero. The primary observed interaction exceeds the null 99.9th percentile (`+0.239` logits; one-sided `p=5.0e-5`, the minimum resolvable with `20,000` permutations), and the content/reasoning extension does too (`+0.155` logits; one-sided `p=5.0e-5`). This tests PT/IT label alignment of the interaction, not a claim that pretrained models lack generic late-stage context dependence.
