...
if run_phase 1; then
    ...
    if [[ -f "results/exp7/unified_acts/merged.npz" ]]; then
        ...
        uv run python -m src.poc.exp07_methodology_validation_tier0.collect_unified_acts --merge-only --n-workers "$NW"
    fi
    ...
    uv run python -m src.poc.exp07_methodology_validation_tier0.collect_unified_acts --verify
...
from src.poc.exp07_methodology_validation_tier0.collect_unified_acts import extract_subset_acts, OUTPUT_DIR
...
import json, numpy as np
from pathlib import Path
from src.poc.exp07_methodology_validation_tier0.collect_unified_acts import extract_subset_acts, OUTPUT_DIR, ALL_LAYERS, D_MODEL
...
# Compute directions
from src.poc.exp07_methodology_validation_tier0.precompute_random_split import OUTPUT_DIR as SPLIT_DIR, compute_directions
