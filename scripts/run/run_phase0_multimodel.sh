...
if [[ "$STEP" == "pca" ]]; then
    ...
    for model in "${models_to_run[@]}"; do
        ...
        uv run python -m src.poc.exp08_multimodel_steering_phase0.pca_rank1 \
...
if [[ "$STEP" == "id-steering" ]]; then
    ...
    for model in "${models_to_run[@]}"; do
        ...
        uv run python -m src.poc.exp08_multimodel_steering_phase0.id_under_steering \
            --model-name "$model" --device "cuda:${gpu}" \
...
if [[ "$STEP" == "commitment" ]]; then
    ...
    uv run python -m src.poc.exp08_multimodel_steering_phase0.commitment_vs_alpha
