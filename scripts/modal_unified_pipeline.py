...
def exp10_collect(model_name: str) -> str:
    ...
    _run([
        "python", "-m", "src.poc.exp10.collect_paired",
        "--model", model_name,
        "--device", "cuda:0",
...
def exp10_probes(model_name: str) -> str:
    ...
    _run([
        "python", "-m", "src.poc.exp10.train_probes",
        "--model", model_name,
...
def exp10_patch_one(model_name: str) -> str:
    ...
    _run([
        "python", "-m", "src.poc.exp10.patching",
        "--model", model_name,
        "--device", "cuda:0",
...
def smoke_test() -> str:
    ...
    from src.poc.exp10.collect_paired import collect_paired_data, commitment_continuous
