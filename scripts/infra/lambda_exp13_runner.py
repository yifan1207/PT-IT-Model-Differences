...
uv sync
bash scripts/run_exp13_exp14_local.sh --mode smoke --model gemma3_4b --smoke-prompts 8 --run-name {shlex.quote(run_name)} --run-root results/exp14_symmetric_matched_prefix_causality/{shlex.quote(run_name)} --smoke-gpu 0 --tuned-lens-dir {shlex.quote(str(remote_probe_dir))}
"""
    else:
        remote_cmd = f"""
set -euo pipefail
...
source .remote_env.sh
uv sync
bash scripts/run_exp13_exp14_local.sh --mode full --n-prompts {n_prompts} --run-name {shlex.quote(run_name)} --run-root results/exp14_symmetric_matched_prefix_causality/{shlex.quote(run_name)} --tuned-lens-dir {shlex.quote(str(remote_probe_dir))}
"""
...
def _sync_results_back(ip: str, ssh_key: Path, run_name: str, *, remote_user: str) -> None:
    local_dir = ROOT / "results" / "exp14_symmetric_matched_prefix_causality" / run_name
    ...
    _run(
        [
            ...
            "-e",
            _rsync_ssh_arg(ssh_key),
            f"{remote_user}@{ip}:{_remote_repo(remote_user)}/results/exp14_symmetric_matched_prefix_causality/{run_name}/",
            f"{local_dir}/",
        ]
    )
...
def main() -> None:
    ...
    try:
        ...
        print(
            "[lambda] results synced to "
            f"results/exp14_symmetric_matched_prefix_causality/{args.run_name}",
            flush=True,
        )
    finally:
