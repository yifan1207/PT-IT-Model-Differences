"""Run the Exp46 fixed-support upstream-state x late-stage 4x4 factorial."""
...
from src.poc.exp46_tulu_fixed_support_stage_sweep import (
...
def run_host_worker(args: argparse.Namespace) -> None:
    ...
    log.info(
        "[exp46] fixed factorial host=%s worker=%d/%d rows=%d",
        host_stage,
        args.worker_index,
        args.n_workers,
        len(support_rows),
    ...
    for idx, row in enumerate(support_rows):
        ...
        for upstream_stage in STAGE_ORDER:
            ...
            if upstream_cache is None:
                log.warning("[exp46] missing upstream cache stage=%s key=%s", upstream_stage, key)
                continue
            ...
            try:
                ...
                log.exception("[exp46] fixed cell host=%s upstream=%s key=%s failed: %s", host_stage, upstream_stage, key, exc)
        if (idx + 1) % 20 == 0:
            log.info("[exp46] fixed host=%s %d/%d", host_stage, idx + 1, len(support_rows))
    ...
    log.info("[exp46] fixed host=%s wrote cells=%d", host_stage, len(records_out))
...
def merge_host(out_dir: Path, host_stage: str, n_workers: int) -> Path:
    ...
    with gzip.open(merged, "wt", encoding="utf-8") as fout:
        for worker_idx in range(n_workers):
            ...
            if not path.exists():
                log.warning("[exp46] missing fixed worker file %s", path)
                continue
