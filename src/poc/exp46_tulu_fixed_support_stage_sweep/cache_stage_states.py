"""Cache Tulu stage full-prefix boundary states and last-token diagnostics."""
...
from src.poc.exp46_tulu_fixed_support_stage_sweep import DEFAULT_BOUNDARY_LAYER
...
def collect_stage_cache(args: argparse.Namespace) -> None:
    ...
    log.info("[exp46] cache stage=%s worker=%d/%d rows=%d", stage, args.worker_index, args.n_workers, len(rows))
    for idx, row in enumerate(rows):
        ...
        try:
            ...
            log.exception("[exp46] cache stage=%s key=%s failed: %s", stage, key, exc)
        if (idx + 1) % 20 == 0:
            log.info("[exp46] cache stage=%s %d/%d", stage, idx + 1, len(rows))
    ...
    log.info("[exp46] cache stage=%s wrote records=%d states=%s", stage, len(out_records), states_path)
...
def merge_stage_cache(out_dir: Path, stage: str, n_workers: int) -> Path:
    ...
    with gzip.open(merged, "wt", encoding="utf-8") as fout:
        for worker_idx in range(n_workers):
            ...
            if not path.exists():
                log.warning("[exp46] missing stage cache record file %s", path)
                continue
