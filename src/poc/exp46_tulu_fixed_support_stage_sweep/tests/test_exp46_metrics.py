...
from src.poc.exp46_tulu_fixed_support_stage_sweep.common import validate_full_prefix_boundary
from src.poc.exp46_tulu_fixed_support_stage_sweep.analyze import (
    _cluster_bootstrap,
    _paired_ratio_bootstrap,
    _unit_effects,
)
from src.poc.exp46_tulu_fixed_support_stage_sweep.support import support_file_name
...
def test_stage_fraction_denominator_gate() -> None:
    numerator = [{"prompt_id": "p0", "event_kind": "first_diff", "interaction": 1.0}]
    denominator = [{"prompt_id": "p0", "event_kind": "first_diff", "interaction": 2.0}]
    out = _paired_ratio_bootstrap(numerator, denominator, metric="interaction", n_boot=20, seed=1)
    assert out["ratio_estimate"] == 0.5
    assert out["n_units"] == 1
    assert out["status"] in {"ok", "denominator_ci_includes_zero_do_not_interpret_as_fraction"}


def test_support_file_names() -> None:
    assert support_file_name("R") == "base_final_first_diff.jsonl.gz"
    assert support_file_name("S") == "base_s_first_diff.jsonl.gz"
    assert support_file_name("D") == "base_d_first_diff.jsonl.gz"
