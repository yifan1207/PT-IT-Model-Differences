"""Fixed-support Exp20-style identity/margin decomposition across Tulu stages."""
...
from src.poc.exp46_tulu_fixed_support_stage_sweep import MODEL_NAME, NON_BASE_STAGES
...
def run_identity_worker(args: argparse.Namespace) -> None:
    ...
    log.info("[exp46] identity target=%s worker=%d/%d rows=%d", target_stage, args.worker_index, args.n_workers, len(rows))
    conditions = [
        ("A_base", "B", None, "none"),
        ...
        for idx, row in enumerate(rows):
            ...
            for name, host, donor, graft_kind in conditions:
                try:
                    ...
                    log.exception("[exp46] identity target=%s condition=%s prompt=%s failed: %s", target_stage, name, row["prompt_id"], exc)
                    payload["conditions"][name] = {"condition": name, "error": str(exc)}
            fout.write(json.dumps(payload, separators=(",", ":")) + "\n")
            fout.flush()
            if (idx + 1) % 20 == 0:
                log.info("[exp46] identity target=%s %d/%d", target_stage, idx + 1, len(rows))
...
def merge_identity(out_dir: Path, target_stage: str, n_workers: int) -> Path:
    ...
    with gzip.open(merged, "wt", encoding="utf-8") as fout:
        for worker_idx in range(n_workers):
            ...
            if not path.exists():
                log.warning("[exp46] missing identity file %s", path)
                continue
