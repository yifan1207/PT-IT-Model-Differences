...
Usage examples:
    modal run src/poc/exp11_matched_prefix_mlp_graft/modal_exp11.py --mode smoke --run-name gemma3_4b_smoke_v2
    modal run src/poc/exp11_matched_prefix_mlp_graft/modal_exp11.py --mode preflight --model gemma3_4b --run-name gemma3_4b_preflight_v1
    modal run src/poc/exp11_matched_prefix_mlp_graft/modal_exp11.py --mode full --model gemma3_4b --run-name gemma3_4b_full_v1
    modal run src/poc/exp11_matched_prefix_mlp_graft/modal_exp11.py --mode full --run-name exp11_full_20260409
"""
...
from src.poc.exp11_matched_prefix_mlp_graft.run import _apply_prompt_shard, _random_subsample, _sample_prompts
...
def _run_exp11(
    *,
    ...
    cmd = [
        "python",
        "-m",
        "src.poc.exp11_matched_prefix_mlp_graft.run",
        "--model",
        model,
        "--dataset",
        dataset_path,
...
def _analyze_exp11(run_dir: str, *, seed: int) -> None:
    _run_subprocess(
        ["python", "-m", "src.poc.exp11_matched_prefix_mlp_graft.analyze", "--run-dir", run_dir],
        seed=seed,
    )
...
def _launch_balanced_10gpu_v11_teacherforced(
    ...
    *,
    ...
    print(
        "IMPORTANT: First deploy with `modal deploy src/poc/exp11_matched_prefix_mlp_graft/modal_exp11.py`, "
