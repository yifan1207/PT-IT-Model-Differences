...
Usage:
    uv run python -m src.poc.exp4.run                        # PT model
    uv run python -m src.poc.exp4.run --variant it           # IT model
    uv run python -m src.poc.exp4.run --variant it --no-attention
    uv run python -m src.poc.exp4.run --dataset data/exp3_dataset.jsonl
    uv run python -m src.poc.exp4.run --variant pt --device cuda:1
...
from src.poc.exp4.collect import collect_all, save_results
...
def main() -> None:
    ...
    if Path(dataset_path).exists():
        ...
        from src.poc.exp2.prompts import PROMPTS as prompts
        prompt_source = "src/poc/exp2/prompts.py (fallback)"
    ...
    print("\nDone.")
    print(f"\nNext steps:")
    print(f"  uv run python -m src.poc.exp4.run_plots --pt-dir {Exp4Config('pt').run_dir} --it-dir {Exp4Config('it').run_dir}")
