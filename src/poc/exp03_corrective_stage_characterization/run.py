...
Usage:
    uv run python -m src.poc.exp03_corrective_stage_characterization.run                          # Format B, PT model
    uv run python -m src.poc.exp03_corrective_stage_characterization.run --variant it             # Format B, IT model
    uv run python -m src.poc.exp03_corrective_stage_characterization.run --variant it --chat-template  # IT-native format
    uv run python -m src.poc.exp03_corrective_stage_characterization.run --variant it --raw-completion # exp 0b
    uv run python -m src.poc.exp03_corrective_stage_characterization.run --gpus 1
    uv run python -m src.poc.exp03_corrective_stage_characterization.run --dataset data/exp3_dataset.jsonl
"""
import argparse
import json
...
from src.poc.exp03_corrective_stage_characterization.collect import collect_all, save_results
...
def main() -> None:
    ...
    if not Path(dataset_path).exists():
        raise FileNotFoundError(
            ...
            f"Build it first:  uv run python -m src.poc.exp03_corrective_stage_characterization.data.build_dataset\n"
