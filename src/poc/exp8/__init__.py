...
Shared infrastructure (not duplicated here):
  scripts/precompute_directions_multimodel.py   Direction extraction (4-phase pipeline)
  scripts/run_phase0_multimodel.sh              Local GPU orchestration
  scripts/modal_phase0.py                       Modal cloud orchestration
  src/poc/exp6/run.py                           Core steering runner (--model-family flag)
  src/poc/exp6/model_adapter.py                 SteeringAdapter for multi-model hooks
  src/poc/cross_model/config.py                 MODEL_REGISTRY (6 models)
  src/poc/cross_model/adapters/                 Per-model architecture adapters

Results: results/exp8/plots/ (PNG) and results/exp8/plots/data/ (JSON/JSONL)
