"""Shared utilities for Exp35 collectors and analysis."""
...
from src.poc.exp35_olmo_base_anchored_stage_decomposition import MODEL_NAME, STAGES
...
def stage_key_for_label(label: str) -> str:
    ...
    if label not in STAGES:
        raise ValueError(f"Unknown OLMo stage {label!r}; expected {list(STAGES)}")
    return label
...
def validate_full_prefix_boundary(tensor: torch.Tensor, *, expected_seq_len: int | None = None) -> None:
    if not torch.is_tensor(tensor) or tensor.ndim != 3:
        raise RuntimeError(
            "Exp35 boundary states must be full-prefix [batch, seq, d_model] tensors; "
    ...
    if expected_seq_len is not None and int(tensor.shape[1]) != int(expected_seq_len):
        raise RuntimeError(
            "Exp35 boundary sequence-length mismatch: "
